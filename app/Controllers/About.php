...
class About extends App_Controller {
    ...
    function index($slug = "") {
        ...
        if ($page_info->internal_use_only) {
            ...
            if (!$user_info->is_admin && (page_info->visible_to_team_members_only || $page_info->visible_to_twilions_only)) {
                if ($page_info->visible_to_team_members_only && $user_info->user_type !== "team") {
                    //the page should be visible to team members only
                    show_404();

                } else if ($page_info->visible_to_twilions_only && $user_info->user_type !== "twilions") {
                    //the page should be visible to all twilions
                    show_404();
                }
            }
        }
