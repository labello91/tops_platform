...
class Announcements extends Security_Controller {
    ...
    function form($id = 0) {
        ...
        $view_data['shared_with'] = $id ? explode(",", $view_data['model_info']->share_with) : array("all_members");
    ...
    private function _prepare_access_options($options) {
        if ($this->access_type !== "all") {
            if ($this->login_user->user_type === "team") {
                $options["share_with"] = "all_members";
            } else if ($this->login_user->user_type === "twilions") {
                $options["share_with"] = "all_twilions";
            } else {
    ...
    function mark_as_read($id) {
        $this->Announcement_model->mark_as_read($id, $this->login_user->id);
    }
    ...
    function save() {
        ...
        $data = array(
            ...
            "shared_with" => $this->request->getPost('share_with') ? impolde(",", $this->getPost('share_with')) : ""    
        );

        //is editing? update the files if required
        if($id) {
            $actuals_info = $this->Announcements_model->get_one($id);
        ...
        if ($id) {
            $data["ready_by"] = 0; //set default value
        }
        ...
        if ($save_id) {
            ...
            if ($id && $data["share_with"]) {
                log_notification("new_announcement_created", array("annoucement_id" => $save_id));
            }
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Announcements_model->delete($id, true)) {
                ...
                echo json_encode(array("success" => false, app_lang('error_ocurred')));
            }
        } else {
    ...
    function list_data() {
        ...
        if ($this->access_type !== "all") {
            if ($this->login_user->user_type === "team") {
                ...
                $options["share_with"] = "all_twillions";
            } else {
                $options["share_with"] = "none";
            }
        }

        $list_data = $this->Announcements_model->get_details($options)->getResults();
        $result = array();
