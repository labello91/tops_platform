...
class BU extends Security_Controller {

    function __construct() {
        parent::__construct();

        //check permission to access this module
        $this->init_permission_checker("bu");
    }
    ...
    function index($tab = "") {
        ...
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("bu", $this->login_user->is_admin, $this->login_user->user_type);

        $view_data['groups_dropdown'] = json_encode($this->_get_groups_dropdown_select2_data(true));
        $view_data['can_edit_bu'] = $this->can_edit_bu();
        $view_data["team_members_dropdown"] = $this->get_team_members_dropdown(true);

        $view_data['tab'] = $tab;

        return $this->template->rander("bu/index", $view_data);
    }

    //for team members, check only read_only permission here, since other permission will be checked accordingly
    private function can_edit_bu() {
    ...
    private function can_add_files() {
        if ($this->login_user->user_type == "team_member") {
            ...
            if (!get_setting("bu_can_add_files")) {
                app_redirect("forbidden");
            }
        }
    }

    private function can_access_this_bu($bu_id = 0) {
        $bu_info = $this->BU_model->get_one($bu_id);

        if ($this->login_user->user_type === "team_member" && $bu_info->$id && get_array_value($this->login_user->permissions, "bu") === "own" && !($bu_info->createdy_by == $this->login_user->id || $bu_info->owner_id == $this->login_user->id)) {
            app_redirect("forbidden");
        }
    }

    /* load bu add/edit modal */
    ...
    function modal_form() {
        ...
        $view_data["view"] = $this->request->getPost('view'); //view='details' needed only when loading from the bu's details view
        $view_data["ticket_id"] = $this->request->getPost('ticket_id'); // needed only when loading from  the ticket's detials view and created by unknown bu
        ...
        //get custom fields
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("bu", $bu_id, $this->login_user->is_admin, $this->login_user->user_type)->getResult();

        return $this->template->view('bu/modal_form', $view_data);
    }

    /* insert or update a bu */
    ...
    function save() {
        ...
        $this->access_only_allowed_members_or_bu_contacts($bu_id);
        ...
        $bu_name = $this->request-getPost('Account_Security', 'Billing', 'Data_Platform', 'Email', 'Enterprise_data_bi', 'Flex', 'Identity', 'Messaging', 'Super_network', 'Platform_engineering', 'Video', 'Voice', 'Wireless', 'Other_bu');
        ...
        if ($this->login_user->user_type === "team_member") {
            $data["group_ids"] = $this->request->getPost('groups_ids') ? $this->request->getPost('group_ids') : "";
        }
        ...
        if ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "bu") === "all") {
            ...
            //the user hasn't permission to change created by but can create a new bu
        ...
        if ($save_id) {
            save_custom_fields("bu", $save_id, $this->login_user->is_admin, $this->login_user->user_type);

            //save bu id on tickets if any tickets id exists
            $ticket_id = $this->request->getPost('ticket_id');
            if ($ticket_id) {
                $ticket_data = array("bu_id" => $save_id);
    ...
    /* list of bus, prepared for datatable */

    function list_data() {
        $this->access_only_allowed_members();
        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("bu", $this->login_user->is_admin, $this->login_user->user_type);
        $options = array(
        ...
        $list_data = $this->BU_model->get_details($options)->getResults();
        $result = array();
    ...
    /* prepare a row of bu list table */
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $primary_contact = get_bu_contact_profile_link($data->primary_contact_id, $contact);

        $group_list = "";
        if ($data->bu_groups) {
            $groups = explode(",", $data->bu_groups);
        ...
        if ($data->forecast_value) {
            $due = ignor_minor_value($data->forecast_value - $data->ytd_actual);
        }

        $row_data = array($data->id,
            anchor(get_uri("bu/view/" . $data->id), $data->bu_name),
            $data->primary_contact ? $primary_contact : "",
            $group_list,
            to_decimal_format($data->total_projects),
            to_currency($data->forecast_value, $data->currency_symbol),
            to_currency($data->ytd_actual, $data->currency_symbol),
        ...
        $row_data[] = modal_anchor(get_uri("bu/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_bu'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_bu'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("bu/delete"), "data-action" => "delete-confirmation"));

        return $row_data;
    }

    /* load bu details view */
    ...
    function view($bu_id = 0, $tab = "") {
        ...
        if ($bu_id) {
            ...
            if ($bu_info && !$bu_info->is_lead) {

                $access_info = $this->get_access_info("forecast");
                $view_data["show_forecast_info"] = (get_setting("module_forecast") && $this->can_view_forecast()) ? true : false;
                ...
                $access_info = $this->get_access_info("actuals");
                $view_data["show_actuals_info"] = (get_setting("module_actuals") && $access_info->access_type == "all") ? true : false;

                $view_data['bu_info'] = $bu_info;

                $view_data["is_starred"] = strpos($bu_info->starred_by, ":" . $this->login_user->id . ":") ? true : false;

                $view_data["tab"] = $tab;

                $view_data["view_type"] = "";

                //even it's hidden, admin can view all information of bu
                $view_data['hidden_menu'] = array("");

                return $this->template->rander("bu/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    /* add-remove start mark from bu */

    function add_remove_star($bu_id, $type = "add") {
        if ($bu_id) {
            $view_data["bu_id"] = $bu_id;

            if ($type === "add") {
                $this->BU_model->add_remove_star($bu_id, $this->login_user->id, $type = "add");
                return $this->template->view('bu/star/starred', $view_data);
            } else {
                $this->BU_model->add_remove_star($bu_id, $this->login_user->id, $type = "remove");
                return $this->template->view('bu/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_bu() {
        $view_data["bu"] = $this->BU_model->get_starred_bu($this->login_user->id)->getResult();
        return $this->template->view('bu/star/bu_list', $view_data);
    }

    /* load projects tab  */

    function projects($bu_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_bu($bu_id);
        ...
        $view_data['bu_id'] = $bu_id;
        return $this->template->view("bu/projects/index", $view_data);
    }

    /* load payments tab  */

    function ytd_actual($bu_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_bu($bu_id);

        if ($bu_id) {
            $view_data["bu_info"] = $this->BU_model->get_one($bu_id);
            $view_data['bu_id'] = $bu_id;
            return $this->template->view("bu/ytd_actual/index", $view_data);
        }
    }

    /* load tickets tab  */

    function tickets($bu_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_bu($bu_id);
        ...
        if ($bu_id) {
            ...
            return $this->template->view("bu/tickets/index", $view_data);
        }
    }
    ...
    function forecast($bu_id) {
        ...
        if ($bu_id) {
            ...
            $view_data["can_edit_forecast"] = $this->can_edit_forecast();

            return $this->template->view("bu/forecast/index", $view_data);
        }
    }
    ...
    function notes($bu_id) {
        ...
        if ($bu_id) {
            $view_data['bu_id'] = $bu_id;
            return $this->template->view("bu/notes/index", $view_data);
        }
    }
    ...
    function events($bu_id) {
        ...
        if ($bu_id) {
            $view_data['cbu_id'] = $bu_id;
            $view_data['calendar_filter_dropdown'] = $this->get_calendar_filter_dropdown("bu");
    ...
    function files($bu_id, $view_type = "") {
        $this->can_view_files();
        $this->can_access_this_bu($bu_id);

        if ($this->login_user->user_type == "bu") {
            $bu_id = $this->login_user->bu_id;
        }
        ...
        if ($view_type == "page_view") {
            ...
            return $this->template->rander("bu/files/index", $view_data);
        } else {
            return $this->template->view("bu/files/index", $view_data);
        }
    }
    ...
    function file_modal_form() {
        ...
        $view_data['bu_id'] = $bu_id;
        return $this->template->view('bu/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        if ($data->uploaded_by_user_type == "tem_member") {
            $uploaded_by = get_team_member_profile_link($data->uploaded_by, $uploaded_by);
        } else {
            $uploaded_by = get_bu_contact_profile_link($data->uploaded_by, $uploaded_by);
        }

        $description = "<div class='float-start'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("bu/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("bu/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        if ($this->login_user->user_type == "team_member") {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("bu/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $this->can_access_this_bu($file_info->bu_id);
            ...
            return $this->template->view("bu/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function delete_file() {
        ...
        if (!$info->bu_id || ($this->login_user->user_type == "bu" && $info->uploaded_by !== $this->login_user->id)) {
            app_redirect("forbidden");
        }

        $this->can_access_this_bu($info->bu_id);
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $this->can_access_this_bu($view_data['user_info']->bu_id);
        $view_data['bu_info'] = $this->BU_model->get_one($view_data['user_info']->bu_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "bu") {

            $view_data['show_contact_info'] = true;
            return $this->template->rander("bu/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        if (!get_setting("disable_language_selector_for_bu")) {
        ...
        return $this->template->view("bu/contacts/my_preferences", $view_data);
    }
    ...
    function save_personal_language($language) {
        if (!get_setting("disable_language_selector_for_bu") && ($language || $language === "0")) {
    ...
    function contacts($bu_id = 0) {
        ...
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("bu_contacts", $this->login_user->is_admin, $this->login_user->user_type);

        $view_data['can_edit_bu'] = $this->can_edit_bu();

        return $this->template->view("bu/contacts/index", $view_data);
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->can_access_this_bu($view_data['model_info']->bu_id);

        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("bu_contacts", $view_data['model_info']->id, $this->login_user->is_admin, $this->login_user->user_type)->getResult();
        return $this->template->view('bu/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->can_access_this_bu($view_data['model_info']->bu_id);
            $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("bu_contacts", $contact_id, $this->login_user->is_admin, $this->login_user->user_type)->getResult();

            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $view_data['can_edit_bu'] = $this->can_edit_bu();
            return $this->template->view('bu/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function save_contact() {
        ...
        //by default, the first contact of a bu is the primary contact
        ...
        if ($save_id) {
            ...
            save_custom_fields("bu_contacts", $save_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    //save account settings of a bu contact (user)
    function save_account_settings($user_id) {
        ...
        if ($this->Users_model->ci_save($account_data, $user_id)) {
            ...
            //resend new password to bu contact
    ...
    function contacts_list_data($bu_id = 0) {
        ...
        $this->access_only_allowed_members_or_bu_contact($bu_id);
        $this->can_access_this_bu($bu_id);

        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("bu_contacts", $this->login_user->is_admin, $this->login_user->user_type);

        $options = array("user_type" => "bu", "bu_id" => $bu_id, "custom_fields" => $custom_fields, "show_own_bu_only_user_id" => $this->show_own_bu_only_user_id());
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false) {
        ...
        if ($this->login_user->user_type == "team_member" && $data->requested_account_removal) {
            $removal_request_pending = "<span class='bg-danger badge'>" . app_lang("removal_request_pending") . "</span>";
        }

        $contact_link = anchor(get_uri("bu/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "bu") {
            $contact_link = $full_name; //don't show clickable link to bu
        }

        $bu_info = $this->BU_model->get_one($data->bu_id);

        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("bu/view/" . $data->bu_id), $bu_info->bu_name),
            $data->job_title,
            $data->email
        );
        ...
        $row_data[] = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("bu/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
        if (get_setting("disable_user_invitation_option_by_bu") && $this->login_user->user_type == "bu") {
            app_redirect("forbidden");
        }
        ...
        $view_data["bu_info"] = $this->BU_model->get_one($bu_id);
        return $this->template->view('bu/contacts/invitation_modal', $view_data);
    }
    ...
    function send_invitation() {
        ...
        $email_template = $this->Email_templates_model->get_final_template("bu_contact_invitation");
    ...
    function users() {
        if ($this->login_user->user_type === "bu") {
            ...
            return $this->template->rander("bu/contacts/users", $view_data);
        }
    }
    ...
    function import_bu_modal_form() {
        $this->access_only_allowed_members();
        if (!$this->can_edit_bu()) {
            app_redirect("forbidden");
        }

        return $this->template->view("bu/import_bu_modal_form");
    }

    private function _prepare_bu_data($data_row, $allowed_headers) {
        //prepare bu data
        $bu_data = array();
        $bu_contact_data = array("user_type" => "bu", "is_primary_contact" => 1);
        ...
        foreach ($data_row as $row_data_key => $row_data_value) { //row values
            ...
            } else if ($header_key_value == "bu_groups") { //we've to make bu groups data differently
                $bu_data["group_ids"] = $this->_get_bu_group_ids($row_data_value);
            } else if ($header_key_value == "contact_first_name") {
                $bu_contact_data["first_name"] = $row_data_value;
            } else if ($header_key_value == "contact_last_name") {
                $bu_contact_data["last_name"] = $row_data_value;
            } else if ($header_key_value == "contact_email") {
                $bu_contact_data["email"] = $row_data_value;
            } else {
                $bu_data[$header_key_value] = $row_data_value;
            }
        }

        return array(
            "bu_data" => $bu_data,
            "bu_contact_data" => $bu_contact_data,
    ...
    function save_bu_from_excel_file() {
        if (!$this->can_edit_bu()) {
            app_redirect("forbidden");
        }

        if (!$this->validate_import_bu_file_data(true)) {
            echo json_encode(array('success' => false, 'message' => app_lang('error_occurred')));
        }
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            $bu_data_array = $this->_prepare_bu_data($value, $allowed_headers);
            $bu_data = get_array_value($bu_data_array, "bu_data");
            $bu_contact_data = get_array_value($bu_data_array, "bu_contact_data");
            $custom_field_values_array = get_array_value($bu_data_array, "custom_field_values_array");

            //couldn't prepare valid data
            if (!($bu_data && count($bu_data))) {
                continue;
            }

            //found information about bu, add some additional info
            $bu_data["created_date"] = $now;
            $bu_data["created_by"] = $this->login_user->id;
            $bu_contact_data["created_at"] = $now;

            //save bu data
            $bu_save_id = $this->BU_model->ci_save($bu_data);
            if (!$bu_save_id) {
                continue;
            }

            //save custom fields
            $this->_save_custom_fields_of_bu($bu_save_id, $custom_field_values_array);

            //add bu id to contact data
            $bu_contact_data["bu_id"] = $bu_save_id;
            $this->Users_model->ci_save($bu_contact_data);
        }
    ...
    private function _save_custom_fields_of_bu($bu_id, $custom_field_values_array) {
    ...
    private function _get_bu_group_ids($bu_groups_data) {
        $explode_bu_groups = explode(", ", $bu_groups_data);
        if (!($explode_bu_groups && count($explode_bu_groups))) {
            return false;
        }

        $groups_ids = "";

        foreach ($explode_bu_groups as $group) {
            $group_id = "";
            $existing_group = $this->bu_groups_model->get_one_where(array("title" => $group, "deleted" => 0));
            if ($existing_group->id) {
                //bu group exists, add the group id
                $group_id = $existing_group->id;
            } else {
                //bu group doesn't exists, create a new one and add group id
                $group_data = array("title" => $group);
                $group_id = $this->bu_groups_model->ci_save($group_data);
            }
    ...
    function validate_import_bu_file() {
        $this->access_only_allowed_members();
    ...
    function validate_import_bu_file_data($check_on_submit = false) {
        $this->access_only_allowed_members();
    ...
    private function _row_data_validation_and_get_error_message($key, $data, $has_contact_first_name) {
        ...
        if ($has_contact_first_name) {
            if ($header_value == "contact_last_name" && !$data) {
                return app_lang("import_bu_error_contact_name");
            }
            ...
            if ($header_value == "contact_email") {
                if ($data) {
                    ...
                    return app_lang("import_bu_error_contact_email");
                }
            }
        }
    }
    ...
    function download_sample_excel_file() {
        $this->access_only_allowed_members();
        return $this->download_app_files(get_setting("system_file_path"), serialize(array(array("file_name" => "import-bu-sample.xlsx"))));
    }

    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        return $this->template->view("bu/contacts/gdpr", $view_data);
    }

    function export_my_data() {
        if (get_setting("enable_gdpr") && get_setting("allow_bu_to_export_their_data")) {
    ...
    private function _make_export_data($user_info) {
        ...
        $data .= "\n\n";
        $data .= strtoupper(app_lang("bu_info")) . "\n";

        //add bu info
        $bu_info = $this->BU_model->get_one($user_info->bu_id);
        $required_bu_info_array = array("bu_name", "city", "country");
        foreach ($required_bu_info_array as $field) {
            if ($bu_info->$field) {
                $data .= app_lang($field) . ": " . $bu_info->$field . "\n";
            }
        }
    }
    
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("bu_can_request_account_removal")) {

            $user_id = $this->login_user->id;
            $data = array("requested_account_removal" => 1);
            $this->Users_model->ci_save($data, $user_id);

            $bu_id = $this->Users_model->get_one($user_id)->bu_id;
            log_notification("bu_contact_requested_account_removal", array("bu_id" => $bu_id), $user_id);

            $this->session->setFlashdata("success_message", app_lang("estimate_submission_message"));
            app_redirect("bu/contact_profile/$user_id/gdpr");
        }
    }
    
    /* load expenses tab  */

    function expenses($bu_id) {
        ...
        if ($bu_id) {
            ...
            return $this->template->view("bu/expenses/index", $view_data);
        }
    }
...
/* End of file BU.php */
/* Location: ./app/controllers/BU.php */