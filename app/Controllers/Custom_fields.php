...
class Custom_fields extends Security_Controller {
    ...
    function save() {
        ...
        $data = array(
            ...
            "show_in_forecast" => $this->request->getPost('show_in_forecast') ? 1 : 0,
            "visible_to_admins_only" => $this->request->getPost('visible_to_admins_only') ? 1 : 0,
            "hide_from_bu" => $this->request->getPost('hide_from_bu') ? 1 : 0,
            "disable_editing_by_bu" => $this->request->getPost('disable_editing_by_bu') ? 1 : 0,
    ...
    function list_data($related_to) {
        // accessible from bu and team members
    ...
    function update_field_sort_values($id = 0) {
        ...
        $sort_values = $this->require->getPost("sort_values");
        if ($sort_values) {

            //extract the values from the comma separated string
            $sort_item = explode(",", $sort_values);
    ...
    function bu_contacts() {
        return $this->template->view('custom_fields/settings/bu_contact');
    }
