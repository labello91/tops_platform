...
class Events extends Security_Controller {
    ...
    private function can_share_events() {
        if ($this->login_user->user_type === "team_members") {
    ...
    function modal_form() {
        ...
        //for a specific share, we have to find that if it's been shared with team member or bu's contact
        ...
        $view_data['bu_id'] = $this->request->getPost('bu_id');

        //don't show bu dropdown for lead's estimate editing
        $bu_info = $this->BU_model->get_one($model_info->bu_id);
        if ($bu_info->is_lead) {
            $view_data['bu_id'] = $bu_info->id;
        }
        ...
        //prepare bu dropdown, check if user has permission to access the bu
        $bu_access_info = $this->get_access_info("bu");

        $bu_dropdown = array();
        if ($this->login_user->is_admin || $bu_access_info->access_type == "all") {
            $bu_dropdown = $this->get_bu_and_leads_dropdown(true);
        }

        $view_data['bu_dropdown'] = $bu_dropdown;
    ...
    function save() {
        ...
        } else if ($share_with == "specific_bu_contacts") {
            $share_with = $this->input->post('share_with_specific_bu_contact');
        }
    ...
    //get calendar event
    function calendar_events($filter_values = "", $event_label_id = 0, $bu_id = 0) {
        $start = $_GET["start"];
        $end = $_GET["end"];
        ...
        if (in_array("events", $filter_values_array)) {
            ...
            $options_of_events = array("user_id" => $this->login_user->id, "team_ids" => $this->login_user->team_ids, "bu_id" => $bu_id, "start_date" => $start, "end_date" => $end, "include_recurring" => true, "is_bu" => $is_bu, "label_id" => $event_label_id);
        ...
        if ($response_by_users) {
            foreach ($response_by_users->getResult() as $user) {
                ...
                if ($user->user_type === "bu") {
                    $profile_link = get_bu_contact_profile_link($user->id, $response_by_user);
                } else {
    ...
    //get all contacts of a selected bu
    function get_all_contacts_of_bu($bu_id) {

        $bu_access_info = $this->get_access_info("bu");
        if ($bu_id && ($this->login_user->is_admin || $bu_access_info->access_type == "all")) {
            $bu_contacts = $this->Users_model->get_all_where(array("status" => "active", "bu_id" => $bu_id, "deleted" => 0))->getResult();
            $bu_contacts_array = array();

            if ($bu_contacts) {
                foreach ($bu_contacts as $contacts) {
                    $bu_contacts_array[] = array("type" => "contact", "id" => "contact:" . $contacts->id, "text" => $contacts->first_name . " " . $contacts->last_name);
                }
            }
            echo json_encode($bu_contacts_array);
        }
    }
    ...
    function save_google_calendar_settings() {
        if (get_setting("enable_google_calendar_api")) {
            $settings = array("integrate_with_google_calendar", "google_bu_id", "google_bu_secret");
            ...
            foreach ($settings as $setting) {
                ...
                if (get_setting('user_' . $this->login_user->id . '_google_calendar_authorized') && ($setting == "google_bu_id" || $setting == "google_bu_secret") && $integrate_with_google_calendar && get_setting('user_' . $this->login_user->id . '_' . $setting) != $value) {
