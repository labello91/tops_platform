...
class External_tickets extends App_Controller {
    ...
    function save() {
        ...
        //match with the existing bu
        $email = $this->request->getPost('email');
        $contact_info = $this->Users_model->get_one_where(array("email" => $email, "user_type" => "bu", "deleted" => 0));

        if ($contact_info->id) {
            //created by existing bu
            $ticket_data["bu_id"] = $contact_info->bu_id;
