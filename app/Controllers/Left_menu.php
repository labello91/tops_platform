 ...
 * Types:
 * "" - Users default
 * "user" - Users custom & BU custom
 * "bu_default" - bu default
 */
 ...
 class Left_menus extends Security_Controller {
    ...
    function index($type = "") {
        ...
        if ($type == "user") {
            ...
            $view_data["setting_active_tab"] = ($type == "bu_default") ? "bu_left_menu" : "left_menu";
            $view_data["type"] = $type;
    ...
    function save() {
        if (get_setting("disable_editing_left_menu_by_bu") && $this->login_user->user_type == "bu") {
        ...
        if ($items_data) {
            ...
            if ($this->login_user->is_admin && $type != "bu_default" && array_search("settings", array_column($items_data, "name")) === false) {
        ...
        if ($type == "user") {
            ...
            if ($type == "bu_default") {
                $this->Settings_model->save_setting("default_bu_left_menu", $items_data);
            } else {
    ...
    private function _prepare_user_custom_redirect_to_url() {
        ...
        if ($this->login_user->user_type == "bu") {
            $redirect_to = "bu/contact_profile/" . $this->login_user->id . "/left_menu";
        }
    ...
    function restore($type = "") {
        ...
        if ($type == "user") {
            ...
            if ($type == "bu_default") {
                $this->Settings_model->save_setting("default_bu_left_menu", "");
                app_redirect("left_menus/index/bu_default");
            } else {
                $this->Settings_model->save_setting("bu_left_menu", "");
