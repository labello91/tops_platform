...
class Messages extends Security_Controller {
    ...
    function modal_form($user_id = 0) {
        ...
        if ($user_id) {
            ...
            foreach ($users as $user) {
                ...
                if ($user->user_type === "bu" && $user->bu_name) { //user is a bu contact
                    if ($this->login_user->user_type == "team_member") {
                        $user_name .= " - " . app_lang("bu") . ": " . $user->bu_name . "";
                    } else {
    ...
    function send_message() {
        ...
        //bu can send messages to only allowed members
    ...
    function chat_list() {
        ...
        if ($this->login_user->user_type === "team_member") {
            ...
            //user is a bu contact and can send messages
            if ($bu_message_users) {
                $view_data['show_users_list'] = true;
            }

            //user can send message to own bu contacts
            if (get_setting("bu_message_own_contacts")) {
                $view_data['show_bu_list'] = true;
            }
        }
