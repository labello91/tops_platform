...
class Notification_processor extends App_Controller {
    ...
    function create_notification($data = array()) {
        ...
        $options = array(
            ...
            "activity_log_id" => get_array_value($data, "activity_log_id"),
            "bu_id" => get_array_value($data, "bu_id"),
            "forecast_id" => get_array_value($data, "forecast_id"),
            ...
            "intake_id" => get_array_value($data, "intake_id")
        );
        ...
        if ($event == "project_task_updated") {
             ...
             * like 'visible to admins only' or 'hide from bu'
    ...
    private function _save_reminder_date(&$event, &$options) {
        //save forecasts reminder dates 
        $forecast_id = get_array_value($options, "forecast_id");
        if ($forecast_id) {
            $forecast_reminder_date = array();
            if ($event == "forecast_due_reminder_before_due_date" || $event == "forecast_overdue_reminder") {
                $forecast_reminder_date["due_reminder_date"] = get_my_local_time();
            }
            if ($event == "recurring_forecast_creation_reminder") {
                $forecast_reminder_date["recurring_reminder_date"] = get_my_local_time();
            }
            if (count($forecast_reminder_date)) {
                $this->Forecast_model->ci_save($forecast_reminder_date, $forecast_id);
            }
        }
