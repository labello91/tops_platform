...
class Projects extends Security_Controller {
    ...
    private function can_view_tasks($project_id = "", $task_id = "") {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_view_tasks")) {
                //even the settings allow to create/edit task, the bu can only create their own project's tasks
                return $this->is_bu_project; 
            }
        }
    }
    ...
    private function can_edit_tasks() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_edit_tasks")) {
                return $this->is_bu_project;
            }
        }
    }
    ...
    private function can_delete_tasks() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_delete_tasks")) {
                return $this->is_bu_project;
            }
        }
    }
    ...
    private function can_comment_on_tasks() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_comment_on_tasks")) {
                //even the settings allow to create/edit task, the bu can only create their own project's tasks
                return $this->is_bu_project;
            }
        }
    ...
    private function can_view_milestones() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_view_milestones")) {
                //even the settings allow to view milestones, the bu can only create their own project's milestones
                return $this->is_bu_project;
            }
        }
    }
    ...
    private function can_delete_files($uploaded_by = 0) {
        if ($this->login_user->user_type == "team_member") {
            ...
            if (get_setting("bu_can_delete_own_files_in_project") && $this->login_user->id == $uploaded_by) {
                return true;
            }
        }
    }
    ...
    private function can_view_files() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_view_project_files")) {
                return $this->is_bu_project;
            }
        }
    }
    ...
    private function can_add_files() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_add_project_files")) {
                return $this->is_bu_project;
            }
        }
    }
    ...
    private function can_comment_on_files() {
        if ($this->login_user->user_type == "team_member") {
            ...
            //check settings for bu's project permission
            if (get_setting("bu_can_comment_on_files")) {
                //even the settings allow to create/edit task, the bu can only comment on their own project's files
                return $this->is_bu_project;
            }
        }
    }
    ...
    private function can_view_gantt() {
        ...
        if (get_setting("module_gantt")) {
            if ($this->login_user->user_type == "team_member") {
                ...
                //check settings for bu's project permission
                if (get_setting("bu_can_view_gantt")) {
                    //even the settings allow to view gantt, the bu can only view on their own project's gantt
                    return $this->is_bu_project;
                }
            }
        }
    ...
    private function init_project_settings($project_id) {
        ...
        foreach ($settings as $setting) {
            config('TOPS')->app_settings_array[$setting->setting_name] = $setting->setting_value;
        }
    }
    ...
    function all_projects($status = "") {
        ...
        if ($this->login_user->user_type === "team_member") {
            ...
            return $this->template->rander("bu/projects/index", $view_data);
        }
    }
    ...
    function modal_form() {
        ...
        if ($bu_id) {
            $view_data['model_info']->bu_id = $bu_id;
        }

        //check if it's from intake. if so, then prepare for project
        $intake_id = $this->request->getPost('intake_id');
        if ($intake_id) {
            $view_data['model_info']->intake_id = $intake_id;
        }
        ...
        $view_data['bu_dropdown'] = $this->BU_model->get_dropdown_list(array("bu_name"), "id", array("is_intake" => 0));
    ...
    function save() {
        ...
        $data = array(
            ...
            "forecast" => unformat_currency($this->request->getPost('forecast')),
            "actuals" => unformat_currency($this->request->getPost('actuals')),
        ...
        //created by bu? overwrite the bu id for safety
        if ($this->login_user->user_type === "bu") {
            $data["bu_id"] = $this->login_user->bu_id;
        }
        ...
        if ($save_id) {
            ...
            if (!$id) {
                ...
                //created from intake? save the project id
                if ($intake_id) {
                    $data = array("project_id" => $save_id);
                    $this->Intakes_model->ci_save($data, $intake_id);
                }
    ...
    function clone_project_modal_form() {
        ...
        $view_data['bu_dropdown'] = $this->BU_model->get_dropdown_list(array("bu_name"), "id", array("is_lead" => 0));

        $view_data['label_suggestions'] = $this->make_labels_dropdown("project", $view_data['model_info']->labels);

        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("projects", $view_data['model_info']->id, 1, "team_member")->getResult(); //we have to keep this regarding as an admin user because non-admin user also can acquire the access to clone a project
    ...
    function save_cloned_project() {
        ...
        if ($new_project_id) {
            ...
            save_custom_fields("projects", $new_project_id, 1, "team_member"); //we have to keep this regarding as an admin user because non-admin user also can acquire the access to clone a project
    ...
    function projects_list_data_of_bu($bu_id = 0) {

        $this->access_only_team_members_or_bu_contact($bu_id);
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $forecast = $data->forecast ? to_currency($data->forecast, $data->currency_symbol) : "-";
        $actuals = $data->actuals ? to_currency($data->actuals, $data->currency_symbol) : "-";
        ...
        $options = "";
        if ($this->can_edit_projects()) {
            $options .= modal_anchor(get_uri("projects/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_project'), "data-post-id" => $data->id));
        }

        if ($this->can_delete_projects()) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_project'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete"), "data-action" => "delete-confirmation"));
        }

        //show the project costs to them who has permission to create projects
        if ($this->login_user->user_type == "team_member" && !$this->can_create_projects()) {
            $forecast = "-";
            $actuals = "-";
        }
        ...
        $row_data = array(
            ...
            $title,
            anchor(get_uri("bu/view/" . $data->bu_id), $data->bu_name),
            $forecast,
            $actuals,
            $data->start_date,
            $start_date,
    ...
    function view($project_id = 0, $tab = "") {
        ...
        $actuals_access_info = $this->get_access_info("actuals");
        $view_data["show_actuals_info"] = (get_setting("module_actuals") && $actuals_access_info->access_type == "all") ? true : false;
        ...
        if ($this->login_user->user_type === "bu") {
            ...
            if (!get_setting("bu_can_edit_projects")) {
    ...
    private function can_access_bu() {
        if (get_setting("bu_can_view_tasks")) {
            if ($this->login_user->is_admin) {
                return true;
            } else if ($this->login_user->user_type == "team_member" && get_array_value($this->login_user->permissions, "bu") && get_array_value($this->login_user->permissions, "show_assigned_tasks_only") !== "1") {
                return true;
            }
        }
    }
    ...
    function add_remove_star($project_id, $type = "add") {
        if ($project_id) {
            ...
            if (get_setting("disable_access_favorite_project_option_for_bu") && $this->login_user->user_type == "bu") {
                app_redirect("forbidden");
            }
    ...
    function overview_for_bu($project_id) {
        if ($this->login_user->user_type === "bu") {
            $view_data = $this->_get_project_info_data($project_id);
            ...
            if (get_setting("bu_can_view_overview")) {
                $view_data['show_overview'] = true;
                $view_data["task_statuses"] = $this->Tasks_model->get_task_statistics(array("project_id" => $project_id));

                if (get_setting("bu_can_view_activity")) {
            ...
            return $this->template->view('projects/overview_for_bu', $view_data);
        }
    }
    ...
    function project_member_modal_form() {
        ...
        if ($add_user_type == "bu_contacts") {
            if (!$this->can_access_bu()) {
                app_redirect("forbidden");
            }

            $contacts = $this->Project_members_model->get_bu_contacts_of_the_project_bu($project_id)->getResult();
    ...
    function project_member_list_data($project_id = 0, $user_type = "") {
        ...
        //show the message icon to bu contacts list only if the user can send message to bu. 
        $can_send_message_to_bu = false;
        $bu_message_users = get_setting("bu_message_users");
        $bu_message_users_array = explode(",", $bu_message_users);
        if (in_array($this->login_user->id, $bu_message_users_array)) {

            $can_send_message_to_bu = true;
        }
        ...
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_project_member_row($data, $can_send_message_to_bu);
        }
        echo json_encode(array("data" => $result));
    }
    ...
    private function _make_project_member_row($data, $can_send_message_to_bu = false) {
        ...
        if ($data->user_type == "team_member") {
            ...
            $member = get_bu_contact_profile_link($data->user_id, $member_image);
            $member_name = get_bu_contact_profile_link($data->user_id, $data->member_name, array("class" => "dark strong"));
        }
        ...
        //check message icon permission for bu contacts
        if (!$can_send_message_to_bu && $data->user_type === "bu") {
            $link = "";
        }
    ...
    private function _make_milestone_row($data) {
        ...
        $options = "";
        if ($this->can_edit_milestones()) {
            $options .= modal_anchor(get_uri("projects/milestone_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_milestone'), "data-post-id" => $data->id));
        }

        if ($this->can_delete_milestones()) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_milestone'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_milestone"), "data-action" => "delete"));
        }
    ...
    function all_tasks() {
        ...
        $assigned_to_list = $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "team_member"));
    ...
    private function get_all_related_data_of_project($project_id, $collaborators = "", $task_labels = "") {
        ...
        if ($project_id) {
            ...
            $project_members = $this->Project_members_model->get_project_members_dropdown_list($project_id, "", $this->can_access_bu())->getResult();
    ...
    function save_task() {
        ...
        //bu can't save the assign to and collaborators
        if ($this->login_user->user_type == "bu") {
    ...
    function save_sub_task() {
        ...
        //don't get assign to id if login user is bu
        if ($this->login_user->user_type == "bu") {
            $data["assigned_to"] = 0;
        } else {
    ...
    function save_task_sort_and_status() {
        ...
        if (($this->login_user->user_type == "team_member" && !can_edit_this_task_status($task_info->assigned_to)) || ($this->login_user->user_type == "bu" && !$this->can_edit_tasks())) {
            app_redirect("forbidden");
        }
    ...
    private function _make_task_row($data, $custom_fields) {
        ...
        if ($data->assigned_to) {
            ...
            if ($data->user_type == "team_member") {
                ...
                $assigned_to = get_bu_contact_profile_link($data->assigned_to, $assigned_to_user);
            }
        }
        ...
        if ($this->login_user->user_type == "team_member" && can_edit_this_task_status($data->assigned_to)) {
            ...
            //don't show clickable checkboxes/status to bu
        ...
        if ($collaborator_list) {
            ...
            foreach ($collaborators_array as $collaborator) {
                ...
                if ($clickable) {
                    ...
                    } else if ($user_type == "bu") {
                        $collaborators .= get_bu_contact_profile_link($collaborator_id, $collaboratr_image, array("title" => $collaborator_name));
                    }
                } else {
    ...
    function twilion_feedback($project_id) {
        $options = array("twilion_feedback_id" => $project_id, "login_user_id" => $this->login_user->id); //twilion feedback id and project id is same
        $view_data['comments'] = $this->Project_comments_model->get_details($options)->getResult();
        $view_data['twilion_feedback_id'] = $project_id;
        $view_data['project_id'] = $project_id;
        return $this->template->view("projects/comments/index", $view_data);
    }
    ...
    function save_comment() {
        ...
        $twilion_feedback_id = $this->request->getPost('twilion_feedback_id');
        ...
        $data = array(
            ...
            "twilion_feedback_id" => $twilion_feedback_id ? $twilion_feedback_id : 0,
        ...
        if ($save_id) {
            ...
            } else if ($comment_info->twilion_feedback_id) {  //twilion feedback comment
                if ($comment_id) {
                    log_notification("project_twilion_feedback_replied", $notification_options);
                } else {
                    log_notification("project_twilion_feedback_added", $notification_options);
                }
    ...
    function download_comment_files($id) {
        ...
        $this->init_project_permission_checker($info->project_id);
        if ($this->login_user->user_type == "bu" && !$this->is_bu_project) {
    ...
    private function _make_file_row($data) {
        ...
        if ($data->uploaded_by_user_type == "team_member") {
    ...
    function history($offset = 0, $log_for = "", $log_for_id = "", $log_type = "", $log_type_id = "") {
        if ($this->login_user->user_type !== "team_member" && ($this->login_user->user_type == "bu" && get_setting("bu_can_view_activity") !== "1")) {
            app_redirect("forbidden");
        }
    ...
    function actuals($project_id) {
        $this->access_only_team_members();
        if ($project_id) {
            $view_data['project_info'] = $this->Projects_model->get_details(array("id" => $project_id))->getRow();
            $view_data['project_id'] = $project_id;
            return $this->template->view("projects/actuals/index", $view_data);
        }
    }

    /* load forecast tab  */

    function forecast($project_id) {
        $this->access_only_team_members();
        if ($project_id) {
            $view_data['project_id'] = $project_id;
            $view_data['project_info'] = $this->Projects_model->get_details(array("id" => $project_id))->getRow();
            ...
            return $this->template->view("projects/forecast/index", $view_data);
        }
    }
    ...
    function change_status($project_id, $status) {
        if ($project_id && $this->can_create_projects() && ($status == "completed" || $status == "hold" || $status == "in_progress" || $status == "canceled" || $status == "open" )) {
    ...
    function gantt_data($project_id = 0, $group_by = "milestones", $milestone_id = 0, $user_id = 0, $status = "") {
        ...
        if ($this->login_user->user_type == "team_member" && !$this->can_manage_all_projects()) {
            $options["user_id"] = $this->login_user->id;
        }
    ...
    function settings_modal_form() {
        ...
        if (!$project_id || !($can_edit_slack_settings)) {
            app_redirect("forbidden");
        }
    ...
    //reset projects dropdown on changing of bu 
    function get_projects_of_selected_bu_for_filter() {
        $this->access_only_team_members();
        $bu_id = $this->request->getPost("bu_id");
        if ($bu_id) {
            $projects = $this->Projects_model->get_all_where(array("bu_id" => $bu_id, "deleted" => 0), "", "", "title")->getResult();
            $projects_dropdown = array(array("id" => "", "text" => "- " . app_lang("project") . " -"));
            foreach ($projects as $project) {
            ...
            //we have show all projects by de-selecting bu
            echo json_encode($this->_get_all_projects_dropdown_list());
        }
    }

    //get bu dropdown
    private function _get_bu_dropdown() {
        $bu_dropdown = array(array("id" => "", "text" => "- " . app_lang("bu") . " -"));
        $bu = $this->BU_model->get_dropdown_list(array("bu_name"), "id", array("is_lead" => 0));
        foreach ($bu as $key => $value) {
            $bu_dropdown[] = array("id" => $key, "text" => $value);
        }
        return $bu_dropdown;
    }
