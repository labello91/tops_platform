...
class Search extends Security_Controller {
    ...
    function get_search_suggestion() {
        ...
        if ($search && $search_field) {
            ...
            } else if ($search_field == "bu") { //bu
                if (!$this->can_access_bu()) {
                    app_redirect("forbidden");
                }
                $options["show_own_bu_only_user_id"] = $this->show_own_buonly_user_id();
                $result = $this->BU_model->get_search_suggestion($search, $options)->getResult();
