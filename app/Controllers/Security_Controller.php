...
class Security_Controller extends App_Controller {
    ...
    protected $is_bu_project = false; //check if loged in user's bu's project
    ...
    protected function get_access_info($group) {
        ...
        if ($this->login_user->is_admin) {
            ...
            if ($module_permission === "all") {
                ...
                if ($group === "forecast" || $group === "actuals" || $group === "team_member_update_permission" || $group == "message_permission") {
    ...
    protected function access_only_allowed_members() {
        ...
        } else if (($this->module_group === "ticket" && ($this->access_type === "specific" || $this->access_type === "assigned_only")) || ($this->module_group === "intake" && $this->access_type === "own") || ($this->module_group === "bu" && ($this->access_type === "own" || $this->access_type === "read_only"))) {
            //can access if it's tickets module and user has a pertial access
            //can access if it's intake module and user has access to own intakes
            //can access if it's bu module and user has a pertial access
            return true;
        } else {
            app_redirect("forbidden");
        }
    ...
    //access only allowed team members or bu contacts 
    protected function access_only_allowed_members_or_bu_contact($bu_id) {
        ...
        } else if ($this->module_group === "bu" && ($this->access_type === "own" || $this->access_type === "read_only")) {
            return true; //can access if it's bu module and user has a pertial access
        } else if ($this->login_user->bu_id === $bu_id) {
            return true; //can access if bu id match 
        } else {
            app_redirect("forbidden");
        }
    }

    //allowed team members and bu himself can access  
    ...
    //access all team members and bu contact
    protected function access_only_team_members_or_bu_contact($bu_id) {
        if (!($this->login_user->user_type === "team_member" || $this->login_user->bu_id === $bu_id)) {
            app_redirect("forbidden");
        }
    }

    //only allowed to access for admin users
    protected function access_only_bu() {
    ...
    protected function can_create_projects() {
        if ($this->login_user->user_type == "team_member") {
            ...
            if (get_setting("bu_can_create_projects")) {
                return true;
            }
        }
    }
    ...
    //access team members and bu
    protected function access_only_team_members_or_bu() {
        if (!($this->login_user->user_type === "team_member" || $this->login_user->user_type === "bu")) {
            app_redirect("forbidden");
        }
    }
    ...
    protected function init_project_permission_checker($project_id = 0) {
        if ($this->login_user->user_type == "bu") {
            ...
            if ($project_info->bu_id == $this->login_user->bu_id) {
                $this->is_bu_project = true;
            }
        } else {
    ...
    protected function can_create_tasks($in_project = true) {
        if ($this->login_user->user_type == "team_member") {
            ...
            if (get_setting("bu_can_create_tasks")) {
                if ($in_project) {
                    //check if it's bu's project
                    return $this->is_bu_project;
                } else {
                    //bu has permission to create tasks on own projects
                    return true;
                }
            }
        }
    ...
    protected function _get_currencies_dropdown() {
        $used_currencies = $this->Forecast_model->get_used_currencies_of_bu()->getResult();
    ...
    protected function get_hidden_topbar_menus_dropdown() {
        ...
        if ($this->login_user->user_type == "team_member") {
            //favourite bu
            $access_bu = get_array_value($this->login_user->permissions, "bu");
            if ($this->login_user->is_admin || $access_bu) {
                array_push($hidden_topbar_menus, "favorite_bu");
            }
    ...
    //get existing projects dropdown for forecast and actuals
    protected function _get_projects_dropdown_for_forecast_and_actuals($type = "all") {
        $projects = $this->Forecast_model->get_used_projects($type)->getResult();
    ...
    protected function _get_groups_dropdown_select2_data($show_header = false) {
        $bu_groups = $this->bu_groups_model->get_all()->getResult();
        $groups_dropdown = array();

        if ($show_header) {
            $groups_dropdown[] = array("id" => "", "text" => "- " . app_lang("bu_groups") . " -");
        }

        foreach ($bu_groups as $group) {
            $groups_dropdown[] = array("id" => $group->id, "text" => $group->title);
        }
        return $groups_dropdown;
    }

    protected function get_bu_and_intakes_dropdown($return_json = false) {
        $bu_dropdown = array("" => "-");
        $bu_json_dropdown = array(array("id" => "", "text" => "-"));
        $bu = $this->BU_model->get_all_where(array("deleted" => 0), 0, 0, "is_intake")->getResult();

        foreach ($bu as $bu) {
            $bu_name = $bu->is_intake ? app_lang("intake") . ": " . $bu->bu_name : $bu->bu_name;

            $bu_dropdown[$bu->id] = $bu_name;
            $bu_json_dropdown[] = array("id" => $bu->id, "text" => $bu_name);
        }

        return $return_json ? $bu_json_dropdown : $bu_dropdown;
    }
    ...
    protected function get_calendar_filter_dropdown($type = "default") {
         ...
         * On bu->events tab, there will be only events and project deadlines field
         * On intake->events tab, there will be only events field
         */
        ...
        if ($type !== "intake") {
            if ($this->login_user->user_type == "team_member" && $type == "default") {
    ...
    protected function check_access_to_store() {
        ...
        if ($this->login_user->user_type == "team_member") {
            ...
            if (!get_setting("bu_can_access_store")) {
                app_redirect("forbidden");
            }
        }
    }
    ...
    protected function get_user_options_for_query($only_type = "") {
         ...
         * bu can only send message to team members and to own contacts (as defined on bu settings)
         * team members can send message to bu (as defined on bu settings)
         */
        ...
        if ($this->login_user->user_type == "team_member") {
            ...
            if (in_array($this->login_user->id, $bu_message_users_array) && $only_type !== "team_member") {
                //user can send message to bu
                $options["member_to_bu"] = true;
            }
        } else {
            //user is a bu contact
            if ($bu_message_users) {
                if ($only_type !== "bu") {
                    $options["bu_to_members"] = $bu_message_users;
                }

                if (get_setting("bu_message_own_contacts") && $only_type !== "team_member") {
                    //bu has permission to send message to own bu contacts
                    $options["bu_id"] = $this->login_user->bu_id;
                }
            }
        }
    ...
    protected function check_access_on_messages_for_this_user() {
        ...
        if ($this->login_user->user_type == "team_member") {
            ...
            if (!$this->login_user->is_admin && get_array_value($this->login_user->permissions, "message_permission") == "no" && !in_array($this->login_user->id, $bu_message_users_array)) {
                $accessable = false;
            }
        } else {
            if (!get_setting("bu_message_users")) {
                $accessable = false;
            }
        }
    ...
    protected function can_view_forecast($bu_id = 0) {
        if ($this->login_user->user_type == "team_member") {
            ...
            if ($this->login_user->bu_id === $bu_id) {
                return true;
            }
        }
    }

    protected function can_edit_forecast() {
        if ($this->login_user->user_type == "team_member" && ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "invoice") === "all")) {
            return true;
        }
    }

    protected function can_access_actuals() {
        $permissions = $this->login_user->permissions;
        if ($this->login_user->is_admin || get_array_value($permissions, "actuals")) {
            return true;
        } else {
            return false;
        }
    ...
    protected function validate_sending_message($to_user_id) {
        ...
        if ($to_user_info->user_type == "team_member") {
    ...
    protected function show_own_bu_only_user_id() {
        if ($this->login_user->user_type === "team_member") {
            return get_array_value($this->login_user->permissions, "bu") == "own" ? $this->login_user->id : false;
        }
    }
    ...
    protected function _get_projects_dropdown() {
        ...
        if ($this->login_user->user_type == "team_member") {
            ...
            $project_options["bu_id"] = $this->login_user->bu_id; //get bu's projects
        }
    ...
    protected function check_access_to_this_item($item_info) {
        if ($this->login_user->user_type === "bu") {
            //check if the item has the availability to show on bu portal
            if (!$item_info->show_in_bu_portal) {
                app_redirect("forbidden");
            }
        }
    }
