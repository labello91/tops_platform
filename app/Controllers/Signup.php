...
class Signup extends App_Controller {
    ...
    function index() {
        //by default only bu can signup directly
        //if bu login/signup is disabled then show 404 page
        if (get_setting("disable_bu_signup")) {
            show_404();
        }

        $view_data["type"] = "bu";
        $view_data["signup_type"] = "new_bu";
        $view_data["signup_message"] = app_lang("create_an_account_as_a_new_bu");

        //check if the email verification before signup is active
        if (get_setting("verify_email_before_bu_signup")) {
    ...
    function accept_invitation($signup_key = "") {
        ...
        if ($valid_key) {
            ...
            if ($type === "staff") {
                ...
                $view_data["signup_message"] = app_lang("create_an_account_as_a_bu_contact");
            }
    ...
    function create_account() {
        ...
        if ($signup_key) {
            ...
            if ($valid_key) {
                ...
                $email = get_array_value($valid_key, "email");
                $type = get_array_value($valid_key, "type");
                $clent_id = get_array_value($valid_key, "bu_id");
                ...
                if ($type === "staff") {
                    ...
                    //check bu id and create bu contact account
                    $bu = $this->BU_model->get_one($clent_id);
                    if (isset($bu->id) && $bu->deleted == 0) {
                        $user_data["bu_id"] = $clent_id;

                        //has any primary contact for this clinet? if not, make this contact as a primary contact
                        $primary_contact = $this->BU_model->get_primary_contact($clent_id);
                        if (!$primary_contact) {
                            $user_data['is_primary_contact'] = 1;
                        }

                        //create a bu contact account
                        $user_id = $this->Users_model->ci_save($user_data);
                        if ($user_id) {
                            log_notification("invited_bu_contact_signed_up", array("bu_id" => $clent_id), $user_id);
                        }
                    } else {
                        //invalid bu
            ...
            //create a bu directly
            if (get_setting("disable_bu_signup")) {
                show_404();
            }

            if (get_setting("verify_email_before_bu_signup") && !$verify_email_key) {
                show_404();
            }
            ...
            $bu_name = $this->request->getPost("bu_name") ? $this->request->getPost("bu_name") : $first_name . " " . $last_name; //save user name as bu name if there is no bu name entered
            ...
            $bu_data = clean_data($bu_data);


            //create a bu
            $bu_id = $this->BU_model->ci_save($bu_data);
            if ($bu_id) {
                //bu created, now create the bu contact
                $user_data["user_type"] = "bu";
                $user_data["email"] = $email;
                ...
                log_notification("bu_signup", array("bu_id" => $bu_id), $user_id);

                //send welcome email
                $email_template = $this->Email_templates_model->get_final_template("new_bu_greetings");
    ...
    function continue_signup($key = "") {
        if ($key && !get_setting("disable_bu_signup")) {
    ...
    private function is_valid_invitation_key($verification_code = "") {
        if ($verification_code) {
            ...
            if ($verification_info && $verification_info->id) {
                ...
                $bu_id = get_array_value($invitation_info, "bu_id");

                if ($email && filter_var($email, FILTER_VALIDATE_EMAIL) && $expire_time && $expire_time > time()) {
                    return array("email" => $email, "type" => $type, "bu_id" => $bu_id);
                }
            }
        }
    }
