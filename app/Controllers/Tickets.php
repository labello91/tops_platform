...
class Tickets extends Security_Controller {
    ...
    function index($status = "") {
        ...
        if ($this->login_user->user_type === "team_member") {
            ...
            return $this->template->rander("bu/tickets/index", $view_data);
        }
    }
    ...
    function modal_form() {
        ...
        //bu should not be able to edit ticket
        if ($this->login_user->user_type === "bu" && $id) {
            app_redirect("forbidden");
        }

        $where = array();
        if ($this->login_user->user_type === "team_member" && $this->access_type !== "all" && $this->access_type !== "assigned_only") {
            $where = array("where_in" => array("id" => $this->allowed_ticket_types));
        }

        $ticket_info = $this->Tickets_model->get_one($this->request->getPost("id"));

        $projects = $this->Projects_model->get_dropdown_list(array("title"), "id", array("bu_id" => $ticket_info->bu_id));
        if ($this->login_user->user_type == "bu") {
            $projects = $this->Projects_model->get_dropdown_list(array("title"), "id", array("bu_id" => $this->login_user->bu_id));
            $ticket_info->bu_id = $this->login_user->bu_id;
        }
        ...
        $view_data['bu_id'] = $ticket_info->bu_id;
        ...
        $assigned_to_dropdown = array("" => "-") + $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "team_member"));
    ...
    //get project suggestion against bu
    function get_project_suggestion($bu_id = 0) {

        $this->access_only_allowed_members();

        $projects = $this->Projects_model->get_dropdown_list(array("title"), "id", array("bu_id" => $bu_id));
        $suggestion = array(array("id" => "", "text" => "-"));
        foreach ($projects as $key => $value) {
            $suggestion[] = array("id" => $key, "text" => $value);
        }
    ...
    function save() {
        ...
        //if this logged in user is a bu then overwrite the bu id
        if ($this->login_user->user_type === "bu") {
        ...
        if ($id) {
            //bu can't update ticket
            if ($this->login_user->user_type === "bu") {
                app_redirect("forbidden");
            }

            //remove not updateable fields
            unset($ticket_data['bu_id']);
    ...
    // list of tickets of a specific bu, prepared for datatable 
    function ticket_list_data_of_bu($bu_id, $is_widget = 0) {
        $this->access_only_allowed_members_or_bu_contact($bu_id);
    ...
    private function _make_row($data, $custom_fields) {
        ...
        } else if ($data->status === "bu_replied" && $this->login_user->user_type === "bu") {
            $data->status = "open"; //don't show bu_replied status to bu
        }
        ...
        $assigned_to = "-";
        if ($data->assigned_to && $this->login_user->user_type == "team_member") {
        ...
        $row_data = array(
            ...
            $title,
            $data->bu_name ? anchor(get_uri("bu/view/" . $data->bu_id), $data->bu_name) : app_lang("unknown_bu"),
        ...
        if ($this->login_user->user_type == "team_member") {
            ...
            if ($ticket_info) {
                $this->access_only_allowed_members_or_bu_contact($ticket_info->bu_id);
    ...
    private function _check_permission_of_selected_ticket($ticket_id = 0) {
        if ($ticket_id && $this->access_type !== "all") {
            ...
            } else if ($this->login_user->user_type === "bu") {
                if ($ticket_info->bu_id != $this->login_user->bu_id) {
                    app_redirect("forbidden");
                }
            } else {
                app_redirect("forbidden");
    ...
    //add bu when there has unknown bu
    function add_bu_modal_form($ticket_id = 0) {
        if ($ticket_id) {
            $this->access_only_allowed_members();

            $view_data['bu_dropdown'] = array("" => "-") + $this->BU_model->get_dropdown_list(array("bu_name"));
            $view_data['ticket_id'] = $ticket_id;

            return $this->template->view("tickets/add_bu_modal_form", $view_data);
        }
    }
    ...
    function link_to_bu() {
        ...
        $data = array("bu_id" => $this->request->getPost("bu_id"));
        $save_id = $this->Tickets_model->ci_save($data, $ticket_id);
    ...
    /* prepare bu contact dropdown based on this suggestion */

    function get_bu_contact_suggestion($bu_id = 0) {

        $this->access_only_allowed_members();

        $bu = $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "bu_id" => $bu_id));
        $suggestion = array(array("id" => "", "text" => "-"));
