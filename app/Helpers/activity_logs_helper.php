...
if (!function_exists('get_change_logs_of_custom_fields')) {
    ...
    function get_change_logs_of_custom_fields($field, $is_notification = false) {
        ...
        $hide_from_bu = get_array_value($explode_custom_fields_data, "3");
        ...
        if ($is_notification && !$visible_to_admins_only) {
            ...
            if (($visible_to_admins_only && !$ci->login_user->is_admin) || ($hide_from_bu && !$visible_to_admins_only && $ci->login_user->user_type == "client")) {
                return false;
            } else {
