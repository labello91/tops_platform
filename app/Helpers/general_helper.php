...
if (!function_exists('get_setting')) {
    ...
    function get_setting($key = "") {
        $setting_value = get_array_value(config('Tops')->app_settings_array, $key);
        if ($setting_value !== NULL) {
            return $setting_value;
        } else {
            if (isset(config('Tops')->$key)) {
                return config('Tops')->$key;
...
/**
 * decode html data which submitted using a encode method of encodeAjaxPostData() function
 * 
 * @param string $html
 * @return htmle
 */
    ...
    function send_app_mail($to, $subject, $message, $optoins = array(), $convert_message_to_html = true) {
        ...
        if (get_setting("email_protocol") === "smtp") {
            ...
            if (!$email_config["SMTPCrypto"]) {
                $email_config["SMTPCrypto"] = "tls"; //for old bu, we have to set this by default
            }
...
if (!function_exists('get_real_ip')) {
    ...
    function get_real_ip() {
        if (!empty($_SERVER['HTTP_bu_IP'])) {
            $ip = $_SERVER['HTTP_bu_IP'];
 ...
 * bu's will see a none clickable link
 * 
 * @param string $id
 * @param string $name
 * @param array $attributes
 ...
 * @return html link
 */
if (!function_exists('get_bu_contact_profile_link')) {

    function get_bu_contact_profile_link($id = 0, $name = "", $attributes = array()) {
        return anchor("bu/contact_profile/" . $id, $name, $attributes);
    }
...
/**
 * return a colorful label according to forecast status
 * 
 * @param Object $forecast_info
 * @return html
 */
if (!function_exists('get_forecast_status_label')) {

    function get_forecast_status_label($forecast_info, $return_html = true) {
        $forecast_status_class = "bg-secondary";
        $status = "not_paid";
        $now = get_my_local_time("Y-m-d");

        //ignore the hidden value. check only 2 decimal place.
        $forecast_info->forecast_value = floor($forecast_info->forecast_value * 100) / 100;

        if ($forecast_info->status == "cancelled") {
            $forecast_status_class = "bg-danger";
            $status = "cancelled";
        } else if ($forecast_info->status != "draft" && $forecast_info->due_date < $now && $forecast_info->actuals_received < $forecast_info->forecast_value) {
            $forecast_status_class = "bg-danger";
            $status = "update_overdue";
        } else if ($forecast_info->status !== "draft" && $forecast_info->actuals_received <= 0) {
            $forecast_status_class = "bg-warning";
            $status = "not_achieved";
        } else if ($forecast_info->actuals_received * 1 && $forecast_info->actuals_received >= $forecast_info->forecast_value) {
            $forecast_status_class = "bg-success";
            $status = "fully_achieved";
        } else if ($forecast_info->actuals_received > 0 && $forecast_info->actuals_received < $forecast_info->forecast_value) {
            $forecast_status_class = "bg-primary";
            $status = "partially_achieved";
        } else if ($forecast_info->status === "draft") {
            $forecast_status_class = "bg-secondary";
            $status = "draft";
        }

        $forecast_status = "<span class='mt0 badge $forecast_status_class large'>" . app_lang($status) . "</span>";
        if ($return_html) {
...
/**
 * get all data to make an forecast
 * 
 * @param Int $forecast_id
 * @return array
 */
if (!function_exists('get_forecast_making_data')) {

    function get_forecast_making_data($forecast_id) {
        $ci = new App_Controller();
        $forecast_info = $ci->Forecast_model->get_details(array("id" => $forecast_id))->getRow();
        if ($forecast_info) {
            $data['forecast_info'] = $forecast_info;
            $data['bu_info'] = $ci->BU_model->get_one($data['forecast_info']->bu_id);
            $data['forecast_items'] = $ci->forecast_items_model->get_details(array("forecast_id" => $forecast_id))->getResult();
            $data['forecast_status_label'] = get_forecast_status_label($forecast_info);
            $data["forecast_total_summary"] = $ci->Forecast_model->get_forecast_total_summary($forecast_id);

            $data['forecast_info']->custom_fields = $ci->Custom_field_values_model->get_details(array("related_to_type" => "forecasts", "show_in_forecast" => true, "related_to_id" => $forecast_id))->getResult();
            $data['bu_info']->custom_fields = $ci->Custom_field_values_model->get_details(array("related_to_type" => "bu", "show_in_forecast" => true, "related_to_id" => $data['forecast_info']->bu_id))->getResult();
            return $data;
        }
    }
 ...
 * get all data to make an forecast
 * 
 * @param Forecast making data $forecast_data
 * @return array
 */
if (!function_exists('prepare_forecast_pdf')) {

    function prepare_forecast_pdf($forecast_data, $mode = "download") {
        $pdf = new Pdf();
        ...
        if ($forecast_data) {
            ...
            $forecast_data["mode"] = $mode;

            $html = view("forecast/forecast_pdf", $forecast_data);
            ...
            $pdf_file_name = app_lang("forecast") . "-" . $forecast_info->id . ".pdf";
 ...
 * @return string
 */
if (!function_exists('get_forecast_id')) {

    function get_forecast_id($forecast_id) {
        $prefix = get_setting("forecast_prefix");
        $prefix = $prefix ? $prefix : strtoupper(app_lang("forecast")) . " #";
        return $prefix . $forecast_id;
    }
...
if (!function_exists('get_estimate_making_data')) {
    ...
    function get_estimate_making_data($estimate_id) {
        ...
        if ($estimate_info) {
            ...
            $data['bu_info'] = $ci->BU_model->get_one($data['estimate_info']->bu_id);
    ...
    function get_order_making_data($order_id = 0) {
        ...
        if ($order_id) {
            ...
            if ($order_info) {
                ...
                $data['bu_info'] = $ci->BU_model->get_one($data['order_info']->bu_id);
            ...
            //count all items of login user (bu)
...
if (!function_exists('log_notification')) {
    ...
    function log_notification($event, $options = array(), $user_id = 0) {
        ...
        if (get_setting("log_direct_notifications")) {
            ...
            $bu = \Config\Services::curlrequest();
            $bu->request('PUT', $url, [
                'query' => $data
            ]);
        }
...
if (!function_exists('save_custom_fields')) {
    ...
    function save_custom_fields($related_to_type, $related_to_id, $is_admin = 0, $user_type = "", $activity_log_id = 0, $save_to_related_type = "", $user_id = 0) {
        ...
        foreach ($custom_fields as $field) {
            ...
            //bu can't edit the field value if the option is active
            if ($user_type == "bu" && $field->disable_editing_by_bu) {
            ...
            if (array_key_exists($field_name, $_POST)) {
                ...
                if ($save_data) {
                    ...
                    $hide_from_bu = get_array_value($changed_values, "hide_from_bu");

                    //add changes of custom fields
                    if (get_array_value($save_data, "operation") == "update") {
                        //update
                        $changes[$field_title . "[:" . $field->id . "," . $field_type . "," . $visible_to_admins_only . "," . $hide_from_bu . ":]"] = array("from" => get_array_value($changed_values, "from"), "to" => get_array_value($changed_values, "to"));
                    } else if (get_array_value($save_data, "operation") == "insert") {
                        //insert
                        $changes[$field_title . "[:" . $field->id . "," . $field_type . "," . $visible_to_admins_only . "," . $hide_from_bu . ":]"] = array("from" => "", "to" => $value);
                    }
                }
            }
        }
...
if (!function_exists('validate_forecast_verification_code')) {

    function validate_forecast_verification_code($code = "", $given_forecast_data = array()) {
        if ($code) {
            $Verification_model = model("App\Models\Verification_model");
            $options = array("code" => $code, "type" => "forecast_actual");
            $verification_info = $Verification_model->get_details($options)->getRow();

            if ($verification_info && $verification_info->id) {
                $existing_forecast_data = unserialize($verification_info->params);

                //existing data
                $existing_forecast_id = get_array_value($existing_forecast_data, "forecast_id");
                $existing_bu_id = get_array_value($existing_forecast_data, "bu_id");
                $existing_contact_id = get_array_value($existing_forecast_data, "contact_id");

                //given data 
                $given_forecast_id = get_array_value($given_forecast_data, "forecast_id");
                $given_bu_id = get_array_value($given_forecast_data, "bu_id");
                $given_contact_id = get_array_value($given_forecast_data, "contact_id");

                if ($existing_forecast_id === $given_forecast_id && $existing_bu_id === $given_bu_id && $existing_contact_id === $given_contact_id) {
                    return true;
                }
            }
        }
...
if (!function_exists('send_message_via_pusher')) {
    ...
    function send_message_via_pusher($to_user_id, $message_data, $message_id, $message_type = "message") {
        ...
        if ($message_type == "message") {
            ...
            if ($pusher->trigger('user_' . $to_user_id . '_message_id_' . $message_id . '_channel', 'tops-chat-event', $data)) {
                return true;
            }
        } else {
            ...
            if ($pusher->trigger('user_' . $to_user_id . '_message_id_' . $message_id . '_channel', 'tops-chat-typing-event', $message_data)) {
                return true;
            }
        }
    }
...
if (!function_exists('can_access_messages_module')) {
    ...
    function can_access_messages_module() {
        ...
        if (($ci->login_user->user_type === "staff" && ($ci->login_user->is_admin || get_array_value($ci->login_user->permissions, "message_permission") !== "no" || in_array($ci->login_user->id, $bu_message_users_array))) || ($ci->login_user->user_type === "bu" && $bu_message_users)) {
            $can_chat = true;
        }
