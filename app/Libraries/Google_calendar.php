...
class Google_calendar {
    ...
    public function __construct() {
        ...
        //load resources
        require_once(APPPATH . "ThirdParty/Google/google-api-php-bu/vendor/autoload.php");
    }

    //authorize connection
    public function authorize($user_id = "") {
        $bu = $this->_get_bu_credentials($user_id);
        $this->_check_access_token($bu, $user_id, true);
    }

    //check access token
    private function _check_access_token($bu, $user_id = "", $redirect_to_settings = false) {
        ...
        if ($accessToken && get_setting('user_' . $user_id . '_google_calendar_authorized')) {
            $bu->setAccessToken(json_decode($accessToken, true));
        }

        // If there is no previous token or it's expired.
        if ($bu->isAccessTokenExpired()) {
            // Refresh the token if possible, else fetch a new one.
            if ($bu->getRefreshToken()) {
                $bu->fetchAccessTokenWithRefreshToken($bu->getRefreshToken());
                $this->_check_calendar_ids($bu, $user_id);
                if ($redirect_to_settings) {
                    app_redirect("events");
                }
            } else {
                $authUrl = $bu->createAuthUrl();
    ...
    private function _check_calendar_ids($bu, $user_id = "") {
        ...
        $service = new \Google_Service_Calendar($bu); //we got the associated user, so directly get the service
    ...
    public function save_access_token($auth_code, $user_id = "") {
        ...
        $accessToken = $bu->fetchAccessTokenWithAuthCode($auth_code);

        $error = get_array_value($accessToken, "error");
        if ($error)
            die($error);

        $bu->setAccessToken($accessToken);

        // Save the token to database
        $new_access_token = json_encode($bu->getAccessToken());
    ...
    //get bu credentials
    private function _get_bu_credentials($user_id = "") {
        $url = get_uri("google_api/save_access_token_of_calendar");

        $bu = new \Google_bu();
        $bu->setApplicationName(get_setting('app_title'));
        $bu->setRedirectUri($url);
        $bu->setAccessType("offline");
        $bu->setPrompt('select_account consent');
        $bu->setbuId(get_setting('user_' . $user_id . '_google_bu_id'));
        $bu->setBUsecret(get_setting('user_' . $user_id . '_google_bu_secret'));
        $bu->setScopes(\Google_Service_Calendar::CALENDAR);
    ...
    private function _get_calendar_service($user_id = 0) {
        $bu = $this->_get_bu_credentials($user_id);
        $this->_check_access_token($bu, $user_id);

        return new \Google_Service_Calendar($bu);
    }
                ...
                foreach ($event->attendees as $attendee) {
                    if ($attendee->email != $user_info->email) {
                        ...
                        if ($share_with_user_info->id) {
                            ...
                            if ($share_with_user_info->user_type == "bu") {
