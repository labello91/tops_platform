...
class Imap {
    ...
    private function _create_ticket_from_imap($message_info = "") {
        if ($message_info) {
            ...
            //check if there has any bu containing this email address
            //if so, go through with the bu id
            $bu_info = $this->ci->Users_model->get_one_where(array("email" => $email, "user_type" => "bu", "deleted" => 0));

            if (get_setting("create_tickets_only_by_registered_emails") && !$bu_info->id) {
                return false;
            }
            ...
            if ($ticket_id) {
                ...
                $ticket_comment_id = $this->_save_tickets_comment($ticket_id, $message_info, $bu_info, true);

                if ($ticket_id && $ticket_comment_id) {
                    log_notification("ticket_commented", array("ticket_id" => $ticket_id, "ticket_comment_id" => $ticket_comment_id, "exclude_ticket_creator" => true), $bu_info->id ? $bu_info->id : "0");
                }
            } else {
                ...
                $ticket_data = array(
                    ...
                    "bu_id" => $bu_info->id ? $bu_info->bu_id : 0,
                    "created_by" => $bu_info->id ? $bu_info->id : 0,
                ...
                if ($ticket_id) {
                    ...
                    $ticket_comment_id = $this->_save_tickets_comment($ticket_id, $message_info, $bu_info, $replying_email);

                    if ($ticket_id && $ticket_comment_id) {
                        log_notification("ticket_created", array("ticket_id" => $ticket_id, "ticket_comment_id" => $ticket_comment_id, "exclude_ticket_creator" => true), $bu_info->id ? $bu_info->id : "0");
                    }
                }
            }
        }
    ...
    private function _save_tickets_comment($ticket_id, $message_info, $bu_info, $is_reply = false) {
        if ($ticket_id) {
            ...
            //add bu_replied status when it's a reply
            if ($is_reply) {
                $ticket_data = array(
                    "status" => "bu_replied",
