...
class Left_menu {
    ...
    private function _get_left_menu_from_setting_for_rander($is_preview = false, $type = "default") {
        ...
        $default_left_menu = ($type == "bu_default" || $this->ci->login_user->user_type == "bu") ? get_setting("default_bu_left_menu") : get_setting("default_left_menu");
        $custom_left_menu = "";
    ...
    private function _get_left_menu_from_setting($type) {
        if ($type == "bu_default") {
            $default_left_menu = get_setting("default_bu_left_menu");
        } else if ($type == "user") {
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "bu_default") {

            $sidebar_menu = array("dashboard" => $dashboard_menu);

            $permissions = $this->ci->login_user->permissions;

            $access_actuals = get_array_value($permissions, "actuals");
            $access_forecast = get_array_value($permissions, "forecast");
            $access_ticket = get_array_value($permissions, "ticket");
            $access_bu = get_array_value($permissions, "bu");
            $access_intakes = get_array_value($permissions, "intakes");

            $bu_message_users = get_setting("bu_message_users");
            $bu_message_users_array = explode(",", $bu_message_users);
            $access_messages = ($this->ci->login_user->is_admin || get_array_value($permissions, "message_permission") !== "no" || in_array($this->ci->login_user->id, $bu_message_users_array));
            ...
            if ($this->ci->login_user->is_admin || $access_bu) {
                $sidebar_menu["bu"] = array("name" => "bu", "url" => "bu", "class" => "briefcase");
            }

            if (get_setting("module_intakes") == "1" && ($this->ci->login_user->is_admin || $access_intakes)) {
                $sidebar_menu["intakes"] = array("name" => "intakes", "url" => "intakes", "class" => "layers");
            }
            ...
            if (get_setting("module_forecast") == "1" && ($this->ci->login_user->is_admin || $access_forecast)) {
                $sidebar_menu["forecasts"] = array("name" => "forecasts", "url" => "forecasts", "class" => "file-text");
            }

            if ($access_items && (get_setting("module_forecast") == "1" || get_setting("module_estimate") == "1" )) {
                $sidebar_menu["items"] = array("name" => "items", "url" => "items", "class" => "list");
            }

            if ((get_setting("module_forecast") == "1" || get_setting("module_actuals") == "1") && ($this->ci->login_user->is_admin || $access_actuals || $access_forecast)) {
                $finance_submenu = array();
                $finance_url = "";
                $show_actuals_menu = false;
                $show_actuals_menu = false;


                if (get_setting("module_forecast") == "1" && ($this->ci->login_user->is_admin || $access_forecast)) {
                    $finance_submenu[] = array("name" => "forecast_actuals", "url" => "forecast_actuals", "class" => "dollar-sign");
                    $finance_url = "forecast_actuals";
                    $show_actuals_menu = true;
                }
                if (get_setting("module_actuals") == "1" && ($this->ci->login_user->is_admin || $access_actuals)) {
                    $finance_submenu[] = array("name" => "actuals", "url" => "actuals", "class" => "dollar-sign");
                    $finance_url = "actuals";
                    $show_actuals_menu = true;
                }

                if ($show_actuals_menu && $show_actuals_menu) {
                    $finance_submenu[] = array("name" => "forecast_vs_actuals", "url" => "actuals/forecast_vs_actuals", "class" => "trending-up");
                }
            ...
            //check message access settings for bu
            if (get_setting("module_message") && get_setting("bu_message_users")) {
            ...
            if (get_setting("module_forecast") == "1") {
                if (!in_array("forecasts", $hidden_menu)) {
                    $sidebar_menu[] = array("name" => "forecasts", "url" => "forecasts", "class" => "file-text");
                }
                if (!in_array("actuals", $hidden_menu)) {
                    $sidebar_menu[] = array("name" => "forecast_actuals", "url" => "forecast_actuals", "class" => "dollar-sign");
                }
            }
            ...
            $sidebar_menu[] = array("name" => "users", "url" => "bu/users", "class" => "users");

            if (get_setting("bu_can_view_files")) {
                $sidebar_menu[] = array("name" => "files", "url" => "bu/files/" . $this->ci->login_user->id . "/page_view", "class" => "image");
            }

            $sidebar_menu[] = array("name" => "my_profile", "url" => "bu/contact_profile/" . $this->ci->login_user->id, "class" => "settings");
