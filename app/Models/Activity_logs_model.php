...
class Actiity_logs_model extends Model {
    ...
    function get_details($options = array()) {
        ...
        if (!$is_admin && $user_id && $user_type !== "bu") {
        ...
        //show bu's own projects activity
        if ($user_type == "bu") {
            $bu_id = get_array_value($options, "bu_id");

            if ($bu_id) {
                $project_join = " LEFT JOIN (SELECT $projects_table.bu_id, $projects_table.id FROM $projects_table WHERE $projects_table.bu_id=$bu_id AND $projects_table.deleted=0 GROUP BY $projects_table.id) AS projects_table ON projects_table.id= $activity_logs_table.log_for_id AND log_for='project' ";
                $project_where = " AND projects_table.bu_id=$bu_id";
            }

            //don't show project's comments to bu
