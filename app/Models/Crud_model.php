...
class Crud_model extends Model {
    ...
    //get query of bu data according to to currency
    protected function _get_bu_of_currency_query($currency, $forecast_table, $bu_table) {
        $default_currency = get_setting("default_currency");
        $currency = $currency ? $currency : $default_currency;

        $bu_where = ($currency == $default_currency) ? " AND $bu_table.currency='$default_currency' OR $bu_table.currency='' OR $bu_table.currency IS NULL" : " AND $bu_table.currency='$currency'";

        return " AND $forecast_table.bu_id IN(SELECT $bu_table.id FROM $bu_table WHERE $bu_table.deleted=0 $bu_where)";
    }

    //get total forecast value calculation query
    protected function _get_forecast_value_calculation_query($forecast_table) {
        $select_forecast_value = "IFNULL(items_table.forecast_value,0)";

        $after_tax_1 = "(IFNULL(tax_table.percentage,0)/100*$select_forecast_value)";
        $after_tax_2 = "(IFNULL(tax_table2.percentage,0)/100*$select_forecast_value)";
        $after_tax_3 = "(IFNULL(tax_table3.percentage,0)/100*$select_forecast_value)";

        $discountable_forecast_value = "IF($forecast_table.discount_type='after_tax', (($select_forecast_value + $after_tax_1 + $after_tax_2) - $after_tax_3), $select_forecast_value )";

        $discount_amount = "IF($forecast_table.discount_amount_type='percentage', IFNULL($forecast_table.discount_amount,0)/100* $discountable_forecast_value, $forecast_table.discount_amount)";

        $before_tax_1 = "(IFNULL(tax_table.percentage,0)/100* ($select_forecast_value- $discount_amount))";
        $before_tax_2 = "(IFNULL(tax_table2.percentage,0)/100* ($select_forecast_value- $discount_amount))";
        $before_tax_3 = "(IFNULL(tax_table3.percentage,0)/100* ($select_forecast_value- $discount_amount))";


        $forecast_value_calculation_query = "(
                $select_forecast_value+
                IF($forecast_table.discount_type='before_tax',  (($before_tax_1+ $before_tax_2) - $before_tax_3), (($after_tax_1 + $after_tax_2) - $after_tax_3))
                - $discount_amount
               )";

        return $forecast_value_calculation_query;
    }
