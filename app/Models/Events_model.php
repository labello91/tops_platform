...
class Events_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($user_id) {
            ...
            $is_bu = get_array_value($options, "is_bu");
            if ($is_bu) {
                //bu user's can't see the events which has shared with all team members
        ...
        if ($bu_id) {
            $where .= " AND $events_table.bu_id=$bu_id";
        }
        ...
        $sql = "SELECT $events_table.*,
            CONCAT($users_table.first_name, ' ',$users_table.last_name) AS created_by_name, $users_table.image AS created_by_avatar, $bu_table.company_name, $bu_table.is_lead, $select_labels_data_query
        FROM $events_table
        LEFT JOIN $users_table ON $users_table.id = $events_table.created_by
        LEFT JOIN $bu_table ON $bu_table.id = $events_table.bu_id    
    ...
    function get_share_with_users_of_event($event_info = "") {
        if ($event_info) {
            ...
            if ($event_info->share_with === "all") {
                $where .= " AND $users_table.user_type = 'team_member' "; //all team members
            } else {
