...
class Messages_model extends Crud_model {
    ...
    function get_users_for_messaging($options = array()) {
        ...
        $member_to_bu = get_array_value($options, "member_to_bu");
        if ($all_members) {
            if ($member_to_bu) {
                $where .= " AND ($users_table.user_type='team_member' OR $users_table.user_type='bu')";
            } else {
                $where .= " AND $users_table.user_type='team_member'";
            }
            ...
            if ($member_to_bu) {
                $where .= " AND ($users_table.id IN($specific_members) OR $users_table.user_type='bu')";
            } else {
                $where .= " AND $users_table.id IN($specific_members)";
            }
        }

        if ($member_to_bu && !$all_members && !$specific_members) {
            $where .= " AND $users_table.user_type='bu'";
        }

        $bu_to_members = get_array_value($options, "bu_to_members");
        $bu_id = get_array_value($options, "bu_id");
        if ($bu_to_members) {
            if ($bu_id) {
                $where .= " AND (FIND_IN_SET($users_table.id, '$bu_to_members') OR $users_table.bu_id=$bu_id)";
            } else {
                $where .= " AND FIND_IN_SET($users_table.id, '$bu_to_members')";
            }
        }

        if ($bu_id && !$bu_to_members) {
