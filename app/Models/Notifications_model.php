...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        $forecast_actuals_id = get_array_value($options, "forecast_actuals_id");
        ...
        $intake_id = get_array_value($options, "intake_id");
         ...
         * team_members, team,
         * project_members, bu_primary_contact, bu_all_contacts, task_assignee, task_collaborators, comment_creator, leave_applicant, ticket_creator, ticket_assignee, post_creator
         */
        ...
        if (in_array("project_members", $notify_to_terms) && $project_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($project_members_table.user_id) AS proje_users FROM $project_members_table WHERE $project_members_table.deleted=0 AND $project_members_table.project_id=$project_id AND $project_members_table.user_id IN (SELECT $users_table.id AS bu_contacts_of_project FROM $users_table WHERE $users_table.deleted=0 AND $users_table.user_type='team_member'))) ";
        }
        ...
        //find bu_all_contacts by project
        if (in_array("bu_all_contacts", $notify_to_terms) && $project_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE FIND_IN_SET($users_table.bu_id, (SELECT $projects_table.bu_id FROM $projects_table WHERE $projects_table.id=$project_id))
                      )
                    ) ";
        }

        //find bu_primary_contacts by project
        if (in_array("bu_primary_contact", $notify_to_terms) && $project_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.is_primary_contact=1 AND FIND_IN_SET($users_table.bu_id, (SELECT $projects_table.bu_id FROM $projects_table WHERE $projects_table.id=$project_id))
                      )
                    ) ";
        }

        //find bu_assigned_contacts by project
        if (in_array("bu_assigned_contacts", $notify_to_terms) && $project_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (
                        SELECT GROUP_CONCAT($project_members_table.user_id) AS proje_users FROM $project_members_table WHERE $project_members_table.deleted=0 AND $project_members_table.project_id=$project_id AND $project_members_table.user_id IN (SELECT $users_table.id AS bu_contacts_of_project FROM $users_table WHERE $users_table.deleted=0 AND $users_table.user_type='bu')
                    )
                  ) ";
        }

        //find bu_all_contacts by ticket
        if (in_array("bu_all_contacts", $notify_to_terms) && $ticket_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE FIND_IN_SET($users_table.bu_id, (SELECT $tickets_table.bu_id FROM $tickets_table WHERE $tickets_table.id=$ticket_id))
                      )
                    ) ";
        }

        //find bu_primary_contacts by project
        if (in_array("bu_primary_contact", $notify_to_terms) && $ticket_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.is_primary_contact=1 AND FIND_IN_SET($users_table.bu_id, (SELECT $tickets_table.bu_id FROM $tickets_table WHERE $tickets_table.id=$ticket_id))
                      )
                    ) ";
        }
        ...
        //find bu_all_contacts by ticket
        if (in_array("bu_all_contacts", $notify_to_terms)) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE FIND_IN_SET($users_table.bu_id))
                    ) ";
        }

        //find bu_primary_contacts by project
        if (in_array("bu_primary_contact", $notify_to_terms)) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.is_primary_contact=1 AND FIND_IN_SET($users_table.bu_id)
                      )
                    ) ";
        }

        //find bu_all_contacts by order
        if (in_array("bu_all_contacts", $notify_to_terms) && $order_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE FIND_IN_SET($users_table.bu_id, (SELECT $orders_table.bu_id FROM $orders_table WHERE $orders_table.id=$order_id))
                      )
                    ) ";
        }

        //find bu_primary_contacts by order
        if (in_array("bu_primary_contact", $notify_to_terms) && $order_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.is_primary_contact=1 AND FIND_IN_SET($users_table.bu_id, (SELECT $orders_table.bu_id FROM $orders_table WHERE $orders_table.id=$order_id))
                      )
                    ) ";
        }

        //find order creator contact if the creator is bu
        if (in_array("order_creator_contact", $notify_to_terms) && $order_id) {
            ...
            $where .= " OR ($users_table.id=( 
                        IF( (SELECT $users_table.user_type FROM $users_table WHERE $users_table.deleted=0 AND $users_table.id=$select_order_creator_query)='bu',
        ...
        //find owner by intake
        if (in_array("owner", $notify_to_terms) && $intake_id) {
            $where .= " OR ($users_table.id=(SELECT $bu_table.owner_id FROM $bu_table WHERE $bu_table.id=$intake_id)) ";
        }
        ...
        if (in_array("recipient", $notify_to_terms) && $event_id) {
            ...
            if ($event_info->share_with === "all") {
                $where .= " OR $users_table.user_type = 'team_member' "; //all team members
            } else {
        ...
        if (in_array("recipient", $notify_to_terms) && $announcement_id) {
            ...
            if (in_array("all_members", $announcement_share_with)) {
                $where .= " OR ($users_table.user_type='team_member' AND $users_table.status='active' AND $users_table.deleted=0)";
            }
            if (in_array("all_bu", $announcement_share_with)) {
                $where .= " OR ($users_table.user_type='bu' AND $users_table.status='active' AND $users_table.deleted=0)";
            }
        }


        //find bu_all_contacts by forecast
        if (in_array("bu_all_contacts", $notify_to_terms) && $forecast_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE FIND_IN_SET($users_table.bu_id, (SELECT $forecast_table.bu_id FROM $forecast_table WHERE $forecast_table.id=$forecast_id))
                      )
                    ) ";
        }

        //find bu_primary_contacts by forecast
        if (in_array("bu_primary_contact", $notify_to_terms) && $forecast_id) {
            $where .= " OR FIND_IN_SET( $users_table.id, (
                        SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.is_primary_contact=1 AND FIND_IN_SET($users_table.bu_id, (SELECT $forecast_table.bu_id FROM $forecast_table WHERE $forecast_table.id=$forecast_id))
                      )
                    ) ";
        }
        ...
        //if it's a bu/intake related notification, we'll check the bu/intake access permission for team members.
        $bu_info = NULL;
        if ($bu_id || $intake_id) {
            $bu_or_intake_id = $bu_id ? $bu_id : $intake_id;
            $bu_info = $this->db->query("SELECT $bu_table.* FROM $bu_table WHERE $bu_table.id=$bu_or_intake_id")->getRow();
        }

        $web_notify_to = "";
        $email_notify_to = "";

        //we've to send email specifically to the unknown bu
        if (get_setting("enable_email_piping")) {
            ...
            if ($ticket_info && !$ticket_info->bu_id && $ticket_info->creator_email &&
        ...
        if ($notify_to->resultID->num_rows) {
            foreach ($notify_to->getResult() as $user) {
                ...
                //check bu/intake permission for team mebers before preparing the notifcation 
                if ($bu_info && !$this->notify_to_this_user_for_this_bu($bu_info, $user)) {
                    continue; //skip next lines for this loop
                }
                ...
                //check if email sending to bu
                if ($user->enable_email_notification && $user->user_type == "bu") {
                    $extra_data["email_sending_to_bu"] = true;
                }
            }
        }
        ...
        $data = array(
            ...
            "bu_id" => $bu_id ? $bu_id : "",
            "forecast_actuals_id" => $forecast_actuals_id ? $forecast_actuals_id : "",
            "forecast_id" => $forecast_id ? $forecast_id : "",
    ...
    //bu will always get notification

    private function notify_to_this_user_for_this_ticket($ticket_info, $user) {

        if ($user->user_type === "bu") {
            return true; //we'll only check the ticket type access permission for team_members
        }
    ...
    private function notify_to_this_user_for_this_task($task_info, $user) {
        ...
        return true; //other users or bu will always get notification
    }

    //if the user has the role to access only own bu/intakes, s/he will get notification where s/he is the creator of the bu/owner of the intake
    //bu will always get notification

    private function notify_to_this_user_for_this_bu($bu_info, $user) {
        if ($user->user_type !== "team_member") {
            return true;
        }
        ...
        //check if user has restriction to view all bu/intakes
        $permissions = unserialize($user->permissions);
        $permissions = is_array($permissions) ? $permissions : array();

        $bu_permission = get_array_value($permissions, "bu");
        if ($bu_info->is_intake) {
            $bu_permission = get_array_value($permissions, "intake");
        }

        if (!($user->is_admin || $bu_permission)) {
            //the user isn't an admin and s/he doesn't have access on any bu
            return false;
        }

        if ($bu_permission == "own" && !($bu_info->owner_id == $user->id || $bu_info->created_by == $user->id)) {
    ...
    function get_notifications($user_id, $offset = 0, $limit = 20) {
        ...
        $forecast_actualss_table = $this->db->prefixTable('forecast_actualss');
        $posts_table = $this->db->prefixTable('posts');
        $forecast_table = $this->db->prefixTable('forecast');
        $bu_table = $this->db->prefixTable('bu');
        $events_table = $this->db->prefixTable('events');
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $notifications_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS user_name, $users_table.image AS user_image,
                 ...
                 $forecast_actualss_table.forecast_id AS actuals_forecast_id, $forecast_actualss_table.amount AS actuals_amount, (SELECT currency_symbol FROM $bu_table WHERE $bu_table.id=$forecast_table.bu_id) AS bu_currency_symbol,
        ...
        LEFT JOIN $forecast_actualss_table ON $forecast_actualss_table.id=$notifications_table.forecast_actuals_id  
        LEFT JOIN $forecast_table ON $forecast_table.id=$notifications_table.forecast_id
