...
class Project_members_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($show_user_wise) {
            if ($user_type == "bu_contacts") {
                $where .= " AND $project_members_table.user_id IN (SELECT $users_table.id FROM $users_table WHERE $users_table.deleted=0 AND $users_table.user_type='bu')";
            } else {
                $where .= " AND $project_members_table.user_id IN (SELECT $users_table.id FROM $users_table WHERE $users_table.deleted=0 AND $users_table.user_type='team_member')";
            }
        }
    ...
    function get_project_members_dropdown_list($project_id = 0, $user_ids = array(), $add_bu_contacts = false, $show_active_users_only = false) {
        $project_members_table = $this->db->prefixTable('project_members');
        $users_table = $this->db->prefixTable('users');
        ...
        $user_where = "";
        if (!$add_bu_contacts) {
            $user_where .= " AND $users_table.user_type='team_member'";
        }
    ...
    function get_rest_team_members_for_a_project($project_id = 0) {
        ...
        WHERE $users_table.user_type='team_member' AND $users_table.status='active' AND $users_table.deleted=0 AND $users_table.id NOT IN (SELECT $project_members_table.user_id FROM $project_members_table WHERE $project_members_table.project_id='$project_id' AND deleted=0)
        ORDER BY $users_table.first_name ASC";

        return $this->db->query($sql);
    }

    function get_bu_contacts_of_the_project_bu($project_id = 0) {
        $project_members_table = $this->db->prefixTable('project_members');
        $users_table = $this->db->prefixTable('users');
        ...
        FROM $users_table
        LEFT JOIN $project_members_table ON $project_members_table.user_id=$users_table.id
        WHERE $users_table.user_type='bu' AND $users_table.deleted=0 AND $users_table.bu_id=(SELECT $projects_table.bu_id FROM $projects_table WHERE $projects_table.id=$project_id) AND $users_table.id NOT IN (SELECT $project_members_table.user_id FROM $project_members_table WHERE $project_members_table.project_id='$project_id' AND deleted=0)
        ORDER BY $users_table.first_name ASC";
