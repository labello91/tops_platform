<?php echo form_open(get_uri("actuals/save"), array("id" => "actuals-form", "class" => "general-form", "role" => "form")); ?>
<div id="actuals-dropzone" class="post-dropzone">
    <div class="modal-body clearfix">
        <div class="container-fluid">
            <input type="hidden" name="id" value="<?php echo $model_info->id; ?>" />
            <div class=" form-group">
                <div class="row">
                    <label for="actuals_date" class=" col-md-3"><?php echo app_lang('date_of_actuals'); ?></label>
                    <div class="col-md-9">
                        <?php
                        echo form_input(array(
                            "id" => "actuals_date",
                            "name" => "actuals_date",
                            "value" => $model_info->actuals_date ? $model_info->actuals_date : get_my_local_time("Y-m-d"),
            ...
            <?php if ($login_user->is_admin || $can_access_bu && $can_access_actuals) { ?>

                <?php if ($bu_id) { ?>
                    <input type="hidden" name="actuals_bu_id" value="<?php echo $bu_id; ?>" />
                <?php } else { ?>
                    <div class="form-group">
                        <div class="row">
                            <label for="actuals_bu_id" class="col-md-3"><?php echo app_lang('bu'); ?></label>
                            <div class="col-md-9">
                                <?php
                                echo form_dropdown("actuals_bu_id", $bu_dropdown, $model_info->bu_id, "class='select2' id='actuals_bu_id'");
                                ?>
                            </div>
                        </div>
                    </div>
            ...
            <div class="form-group">
                <div class="row">
                    <label for="actuals_project_id" class=" col-md-3"><?php echo app_lang('project'); ?></label>
                    <div class=" col-md-9">
                        <?php
                        echo form_dropdown("actuals_project_id", $projects_dropdown, $model_info->project_id, "class='select2 validate-hidden' id='actuals_project_id'");
                        ?>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="row">
                    <label for="actuals_user_id" class=" col-md-3"><?php echo app_lang('team_member'); ?></label>
                    <div class="col-md-9">
                        <?php
                        echo form_dropdown("actuals_user_id", $members_dropdown, $model_info->user_id, "class='select2 validate-hidden' id='actuals_user_id'");
                        ?>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="row">
                    <label for="actuals_recurring" class=" col-md-3"><?php echo app_lang('recurring'); ?>  <span class="help" data-bs-toggle="tooltip" title="<?php echo app_lang('cron_job_required'); ?>"><i data-feather="help-circle" class="icon-16"></i></span></label>
                    <div class=" col-md-9">
                        <?php
                        echo form_checkbox("recurring", "1", $model_info->recurring ? true : false, "id='actuals_recurring' class='form-check-input'");
                        ?>                       
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var uploadUrl = "<?php echo get_uri("actuals/upload_file"); ?>";
        var validationUrl = "<?php echo get_uri("actuals/validate_actuals_file"); ?>";

        var dropzone = attachDropzoneWithForm("#actuals-dropzone", uploadUrl, validationUrl);

        $("#actuals-form").appForm({
            ...
            onAjaxSuccess: function (result) {
                if (!result.success && result.next_recurring_date_error) {
                    ...
                    $("#actuals-form").data("validator").showErrors({
        ...
        setDatePicker("#actuals_date");

        $("#actuals-form .select2").select2();

        $('[data-bs-toggle="tooltip"]').tooltip();

        //show/hide recurring fields
        $("#actuals_recurring").click(function () {
            if ($(this).is(":checked")) {
