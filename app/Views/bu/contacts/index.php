    ...
    <div class="card rounded-0">
        <div class="tab-title clearfix">
            ...
            <div class="title-button-group">
                <?php
                if ($can_edit_bu) {
                    echo modal_anchor(get_uri("bu/invitation_modal"), "<i data-feather='mail' class='icon-16'></i> " . app_lang('send_invitation'), array("class" => "btn btn-default", "title" => app_lang('send_invitation'), "data-post-bu_id" => $bu_id));

                    echo modal_anchor(get_uri("bu/add_new_contact_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_contact'), array("class" => "btn btn-default", "title" => app_lang('add_contact'), "data-post-bu_id" => $bu_id));
                }
                ?>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#contact-table").appTable({
            source: '<?php echo_uri("bu/contacts_list_data/" . $bu_id) ?>',
            order: [[1, "asc"]],
            columns: [
                {title: '', "class": "w50 text-center"},
                {title: "<?php echo app_lang("name") ?>", "class": "w150"},
                {visible: showBuName, title: "<?php echo app_lang("company_name") ?>", "class": "w150"},
