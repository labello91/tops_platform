...
<div class="modal-body clearfix">
    <div class="container-fluid">
        ...
        <?php } else if (count($bu_dropdown)) { ?>
            <div class="form-group">
                <div class="row">
                    <label for="bu_id" class=" col-md-3"><?php echo app_lang('bu'); ?></label>
                    <div class=" col-md-9">
                        <?php
                        echo form_input(array(
                            "id" => "bu_dropdown",
                            "name" => "bu_id",
                            "value" => $model_info->bu_id,
                            "class" => "form-control"
                        ));
                        ?>
                    </div>
        ...
        <?php if ($can_share_events) { ?>
            <?php if ($login_user->user_type == "bu") { ?>
                ...
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class=" col-md-9">
                            ...
                            <div id="share-with-bu-contact" class="form-group mb0 hide">
                                <?php
                                echo form_radio(array(
                                    "id" => "share_with_bu_contact_radio_button",
                                    "name" => "share_with",
                                    "value" => "specific_bu_contacts",
                                    "class" => "toggle_specific form-check-input",
                                ...
                                ?>
                                <label for="share_with_bu_contact_radio_button"><?php echo app_lang("specific_bu_contacts"); ?>:</label>
                                <div class="specific_dropdown" style="display: none;">
                                    <input type="text" value="<?php echo ($model_info->share_with && $model_info->share_with != "all" && $model_info->share_with_specific != "member") ? $model_info->share_with : ""; ?>" name="share_with_specific_bu_contact" id="share_with_specific_bu_contact" class="w100p validate-hidden"  data-rule-required="true" data-msg-required="<?php echo app_lang('field_required'); ?>" placeholder="<?php echo app_lang('choose_bu_contacts'); ?>"  />
                                </div>
                            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var buId = "<?php echo $model_info->bu_id; ?>";

        if (buId && buId != "0") {
            prepareShareWithClientContactsDropdown(buId);
        }

        //show the specific bu contacts readio button after select any bu
        $('#bu_dropdown').select2({data: <?php echo json_encode($bu_dropdown); ?>}).on("change", function () {
            prepareShareWithClientContactsDropdown($(this).val());
        });

        function prepareShareWithClientContactsDropdown(buId) {
            //don't show bu contacts section if the holiday is checked
            if (buId) {
                $("#share-with-bu-contact").removeClass("hide");
                $.ajax({
                    url: "<?php echo get_uri("events/get_all_contacts_of_bu") ?>" + "/" + buId,
                    dataType: "json",
                    success: function (result) {

                        if (result.length) {
                            get_specific_dropdown($("#share_with_specific_bu_contact"), result);
                        } else {
                            //if no bu contact exists, then don't show the share with bu contacts option
                            $("#share-with-bu-contact").addClass("hide");
                ...
                if ($element.val() === "specific_bu_contacts") {
                    //unselect the specific_bu_contacts
        ...
        function toggle_specific_dropdown() {
            ...
            var $element = $(".toggle_specific:checked");
            if ($element.val() === "specific" || $element.val() === "specific_bu_contacts") {
