<?php echo form_open(get_uri("intake/save_file"), array("id" => "file-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <div class="container-fluid">
        <input type="hidden" name="bu_id" value="<?php echo $bu_id; ?>" />
        <?php
        echo view("includes/multi_file_uploader", array(
            "upload_url" => get_uri("intake/upload_file"),
            "validation_url" => get_uri("intake/validate_file"),
        ));
        ?>
    </div>
</div>
    ...
    $(document).ready(function () {
        $("#file-form").appForm({
            onSuccess: function (result) {
                $("#bu-file-table").appTable({reload: true});
            }
        });
