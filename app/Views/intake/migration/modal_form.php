<?php echo form_open(get_uri("intake/save_as_bu"), array("id" => "make-bu-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <div class="container-fluid">
        <?php $model_info = get_array_value($intake_info, "model_info"); ?>
        <input type="hidden" name="main_bu_id" value="<?php echo $model_info->id; ?>" />

        <?php if ($contacts) { ?>
            <div class="form-widget">
                <div>
                    <div class="widget-title row">
                        <div id="intake-info-label" class="col-sm-6"><i data-feather="circle" class="icon-16"></i><strong> <?php echo app_lang('bu_details'); ?></strong></div>
                        <div id="intake-contacts-label" class="col-sm-6"><i data-feather="circle" class="icon-16"></i><strong>  <?php echo app_lang('bu_contacts'); ?></strong></div>
                    </div>
        ...
        <div class="tab-content <?php echo $contacts ? "mt15" : ""; ?>">
            <div role="tabpanel" class="tab-pane active" id="intake-info-tab">
                <?php echo view("bus/bu_form_fields", $intake_info); ?>
                <?php echo view("intake/custom_field_migration", $intake_info); ?>
            </div>

            <div role="tabpanel" class="tab-pane settings" id="intake-contacts-tab">            
                <?php foreach ($contacts as $key => $value) { ?>
                    ...
                    $key = $key + 1;
                    ?>
                    <div class="intake-migration-contacts">
                        ...
                        <div id='contact-<?php echo $key; ?>' class='p15 b-t collapse <?php echo $collapse_in; ?>'>
                            <?php echo view("intake/contacts/contact_general_info_fields_for_migration", array("model_info" => $value, "custom_fields" => $value->custom_fields)); ?>
                        </div>
                    </div>
                <?php } ?>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#make-bu-form input").keydown(function (e) {
            if (e.keyCode === 13) {
                e.preventDefault();
        ...
        function triggerSubmit() {
            ...
            $(".intake-migration-contacts div.collapse").collapse("show");

            $("#make-bu-form").trigger('submit');
        }

        $("#form-previous").click(function () {
            var $infoTab = $("#intake-info-tab"),
                    $contactsTab = $("#intake-contacts-tab"),
        ...
        $("#form-next").click(function () {
            ...
            if ($infoTab.hasClass("active")) {
                ...
                $("#intake-info-label").find("svg").remove();
                $("#intake-info-label").prepend('<i data-feather="check-circle" class="icon-16"></i>');
                feather.replace();
            }
        });
        ...
        var $isPrimaryContactCheckbox = $(".is_primary_contact_intake");
