...
if (!function_exists("make_project_tabs_data")) {
    ...
    function make_project_tabs_data($default_project_tabs = array(), $is_bu = false) {
        $project_tab_order = get_setting("project_tab_order");
        $project_tab_order_of_bu = get_setting("project_tab_order_of_bu");
        $custom_project_tabs = array();

        if ($is_bu && $project_tab_order_of_bu) {
            //user is bu
            $custom_project_tabs = explode(',', $project_tab_order_of_bu);
        } else if (!$is_bu && $project_tab_order) {
            //user is team member
            ...
            <div class="col-md-12">
                <div class="page-title no-bg clearfix mb5 no-border">
                    <div>
                        <h1 class="pl0">
                            ...
                            <?php } else if ($project_info->status == "in_progress") { ?>
                                <span title="<?php echo app_lang("in_progress"); ?>"><i data-feather="check-circle" class='icon'></i></span>
                            <?php } ?>

                            <?php echo $project_info->title; ?>

                            <?php if (!(get_setting("disable_access_favorite_project_option_for_bu") && $login_user->user_type == "bu")) { ?>
                                <span id="star-mark">
                                    <?php
                                    if ($is_starred) {
                ...
                <ul id="project-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs rounded classic mb20 scrollable-tabs border-white" role="tablist">
                    ...
                    if ($login_user->user_type === "team_member") {
                        ...
                        $project_tabs["staff_feedback"] = "projects/staff_feedback/" . $project_info->id;

                        if ($show_forecast_info) {
                            $project_tabs["forecast"] = "projects/forecast/" . $project_info->id;
                            $project_tabs["actuals"] = "projects/actuals/" . $project_info->id;
                        }

                        if ($show_actuals_info) {
                            $project_tabs["actuals"] = "projects/actuals/" . $project_info->id;
                        }
                        ...
                        //default tab order
                        $project_tabs = array(
                            "overview" => "projects/overview_for_bu/" . $project_info->id
                        );
                        ...
                        $project_tabs["comments"] = "projects/staff_feedback/" . $project_info->id;
                ...
                <div class="tab-content">
                    ...
                    <div role="tabpanel" class="tab-pane fade" id="project-staff_feedback-section"></div>
                    <div role="tabpanel" class="tab-pane fade" id="project-notes-section"></div>
                    <div role="tabpanel" class="tab-pane fade" id="project-forecast-section"></div>
                    <div role="tabpanel" class="tab-pane fade" id="project-actuals-section"></div>
                    <div role="tabpanel" class="tab-pane fade" id="project-actuals-section"></div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        setTimeout(function () {
            ...
            } else if (tab === "staff_feedback") {
                $("[data-bs-target='#project-staff_feedback-section']").trigger("click");
            } else if (tab === "files") {
