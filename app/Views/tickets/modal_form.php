...
<div id="new-ticket-dropzone" class="post-dropzone">
    <div class="modal-body clearfix">
        <div class="container-fluid">
            ...
            <!-- bu can't be changed during editing -->
            <?php if ($bu_id) { ?>
            ...
            <?php } else if (!$model_info->creator_email) { ?>
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-md-9">
                            <?php
                            echo form_dropdown("bu_id", $bu_dropdown, array(""), "class='select2 validate-hidden' id='bu_id' data-rule-required='true', data-msg-required='" . app_lang('field_required') . "'");
                            ?>
                        </div>
                    </div>
                </div>
...
<?php if ($show_project_reference == "1") { ?>
            ...
            $("#bu_id").select2().on("change", function () {
                var bu_id = $(this).val();
                if ($(this).val()) {
                    ...
                    $.ajax({
                        url: "<?php echo get_uri("tickets/get_project_suggestion") ?>" + "/" + bu_id,
                        dataType: "json",
...
<?php } ?>
        ...
        //load all bu contacts of selected bu
        $("#bu_id").select2().on("change", function () {
            var bu_id = $(this).val();
            if ($(this).val()) {
                ...
                $.ajax({
                    url: "<?php echo get_uri("tickets/get_bu_contact_suggestion") ?>" + "/" + bu_id,
                    dataType: "json",
                    success: function (result) {
