<div class="bg-white p15 pt0 b-b">
    ...
    if ($ticket_info->status === "bu_replied" && $login_user->user_type === "bu") {
        $ticket_info->status = "open"; //don't show bu_replied status to bu
    }

    $ticket_status = "<span class='badge $ticket_status_class large'>" . app_lang($ticket_info->status) . "</span> ";
    echo $ticket_status;
    ?>
    <?php if ($login_user->user_type === "team_member" && $ticket_info->bu_id) { ?>
        <span class="text-off ml15"><?php echo app_lang("bu") . ": "; ?></span>
        <?php echo $ticket_info->company_name ? anchor(get_uri("bu/view/" . $ticket_info->bu_id), $ticket_info->company_name) : "-"; ?>

        <?php if ($ticket_info->requested_by) { ?>
            <span class="text-off ml15"><?php echo app_lang("requested_by") . ": "; ?></span>
            <?php echo anchor(get_uri("bu/contact_profile/" . $ticket_info->requested_by), $ticket_info->requested_by_name); ?>
        <?php } ?>
    ...
    <?php
    if ($ticket_info->assigned_to && $login_user->user_type == "team_member") {
