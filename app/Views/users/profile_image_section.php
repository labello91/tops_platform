<div class="box" id="profile-image-section">
    <div class="box-content w200 text-center profile-image">
        ...
        //set url
        if ($user_info->user_type === "bu") {
            $url = "bu";
        } else if ($user_info->user_type === "intake") {
            $url = "intake";
        }
    ...
    <div class="box-content pl15">
        ...
        <div class="p10 m0 clearfix">
            ...
            if ($user_info->id != $login_user->id) {
                ...
                //don't show message button in bu contact's page if user hasn't permission to send/receive message to/from bu
                if ($user_info->user_type === "bu") {
                    $bu_message_users = get_setting("bu_message_users");
                    $bu_message_users_array = explode(",", $bu_message_users);
                    if (!in_array($login_user->id, $bu_message_users_array)) {
                        $show_message_button = false;
                    }
                } else if ($user_info->user_type === "intake") {
                    //don't show message button for intake contacts
                    $show_message_button = false;
                }
